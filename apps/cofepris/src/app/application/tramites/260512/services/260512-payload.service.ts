...
export class Shared260512Service {

  /**
 * @description
 * Constructor del servicio `Shared260512Service`.
  ...
  getAllState(): Observable<Record<string, unknown>> {
    return combineLatest([
      ...
      map(([datosDomicilioLegal, tercerosFabricante, solicitudPagoBanco, avisocalidad]) =>
        Shared260512Service.mergeStates(
 ...
 */
  ...
  buildPayload(data: Record<string, unknown>, discriminatorValue: number): Record<string, unknown> {
    const ESTABLECIMIENTO = Shared260512Service.buildEstablecimiento(data);
    const DATOS_SCIAN = Shared260512Service.buildDatosScian(data);
    const MERCANCIAS = Shared260512Service.buildMercancias(data);
    const REPRESENTANTE_LEGAL = Shared260512Service.buildRepresentanteLegal(data);
    const PROVEEDOR_TABLA = Shared260512Service.buildTercerosTablaDatos(data['Proveedor'] as Array<{ tbodyData?: Array<unknown> }>);
    const FORMULADOR_TABLA = Shared260512Service.buildTercerosTablaDatos(data['Formulador'] as Array<{ tbodyData?: Array<unknown> }>);
    const FABRICANTE_TABLA = Shared260512Service.buildTercerosTablaDatos(data['Fabricante'] as Array<{ tbodyData?: Array<unknown> }>);
    const PAGO_DERECHOS = Shared260512Service.buildPagoDerechos(data);
