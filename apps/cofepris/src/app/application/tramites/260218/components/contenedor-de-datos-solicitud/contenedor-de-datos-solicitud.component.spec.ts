...
describe('ContenedorDeDatosSolicitudComponent', () => {
  ...
  let mockTramite260218Query: jest.Mocked<Tramite260218Query>;
  let mockTramite260218Store: jest.Mocked<Tramite260218Store>;

  beforeEach(() => {
    mockTramite260218Query = {
      selectTramiteState$: of({
        opcionConfigDatos: [],
        scianConfigDatos: [],
        tablaMercanciasConfigDatos: [],
      }) as any, // Ensure compatibility with the expected type
    } as unknown as jest.Mocked<Tramite260218Query>; // Cast to jest.Mocked type

    mockTramite260218Store = {
    ...
    } as unknown as jest.Mocked<Tramite260218Store>; // Cast to jest.Mocked type

    TestBed.configureTestingModule({
      declarations: [],
      imports: [CommonModule, DatosDeLaSolicitudComponent,HttpClientTestingModule],
      providers: [
        { provide: Tramite260218Query, useValue: mockTramite260218Query },
        { provide: Tramite260218Store, useValue: mockTramite260218Store },
  ...
  it('opcionSeleccionado should call updateOpcionConfigDatos on the store', () => {
    ...
    expect(mockTramite260218Store.updateOpcionConfigDatos).toHaveBeenCalledWith(mockEvent);
  });
  ...
  it('scianSeleccionado should call updateScianConfigDatos on the store', () => {
    ...
    expect(mockTramite260218Store.updateScianConfigDatos).toHaveBeenCalledWith(mockEvent);
  });
  ...
  it('mercanciasSeleccionado should call updateTablaMercanciasConfigDatos on the store', () => {
    ...
    expect(mockTramite260218Store.updateTablaMercanciasConfigDatos).toHaveBeenCalledWith(mockEvent);
  });
  ...
  it('datasolicituActualizar should call updateDatosSolicitudFormState on the store', () => {
    ...
    expect(mockTramite260218Store.updateDatosSolicitudFormState).toHaveBeenCalledWith(mockEvent);
  });
  ...
  it('datosDeTablaSeleccionados should call update on the store with correct data', () => {
    ...
    expect(mockTramite260218Store.update).toHaveBeenCalled();
  });
