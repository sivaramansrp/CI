...
import {
  ...
  ID_PROCEDIMIENTO,
  PRODUCTO_TABLA_ESTUPEFACIENTES_EXPORTICON,
 ...
 * @description
 * Componente que actúa como contenedor para manejar los datos de la solicitud del trámite 260302.
...
export class ContenedorDeDatosSolicitudComponent implements OnInit, OnDestroy {
   ...
   * @description
   * Estado actual del trámite 260302.
   */
   ...
   *
   * Actualiza la configuración de datos en el store `tramite260302Store`
   * con las opciones seleccionadas.
   */
  opcionSeleccionado(event: TablaOpcionConfig[]): void {
    this.tramite260302Store.updateOpcionConfigDatos(event);
  }
   ...
   *
   * Este método actualiza los datos de configuración SCIAN en el estado del trámite 260302
   * utilizando el evento proporcionado.
   */
  scianSeleccionado(event: TablaScianConfig[]): void {
    this.tramite260302Store.updateScianConfigDatos(event);
  }
  ...
  mercanciasSeleccionado(event: TablaMercanciasDatos[]): void {
    this.tramite260302Store.updateTablaMercanciasConfigDatos(event);
  }
  ...
  datasolicituActualizar(event: DatosSolicitudFormState): void {
    this.tramite260302Store.updateDatosSolicitudFormState(event);
  }
