...
describe('ContenedorDeDatosSolicitudComponent', () => {
  ...
  let mockTramite260304Query: jest.Mocked<Tramite260304Query>;
  let mockTramite260304Store: jest.Mocked<Tramite260304Store>;

  beforeEach(() => {
    mockTramite260304Query = {
      selectTramiteState$: of({
        opcionConfigDatos: [],
        scianConfigDatos: [],
        tablaMercanciasConfigDatos: [],
      }) as any, // Ensure compatibility with the expected type
    } as unknown as jest.Mocked<Tramite260304Query>; // Cast to jest.Mocked type

    mockTramite260304Store = {
    ...
    } as unknown as jest.Mocked<Tramite260304Store>; // Cast to jest.Mocked type

    TestBed.configureTestingModule({
      declarations: [],
      imports: [CommonModule, DatosDeLaSolicitudComponent, ContenedorDeDatosSolicitudComponent,HttpClientModule],
      providers: [
        { provide: Tramite260304Query, useValue: mockTramite260304Query },
        { provide: Tramite260304Store, useValue: mockTramite260304Store },
  ...
  it('opcionSeleccionado should call updateOpcionConfigDatos on the store', () => {
    ...
    expect(mockTramite260304Store.updateOpcionConfigDatos).toHaveBeenCalledWith(mockEvent);
  });
  ...
  it('scianSeleccionado should call updateScianConfigDatos on the store', () => {
    ...
    expect(mockTramite260304Store.updateScianConfigDatos).toHaveBeenCalledWith(mockEvent);
  });
  ...
  it('mercanciasSeleccionado should call updateTablaMercanciasConfigDatos on the store', () => {
    ...
    expect(mockTramite260304Store.updateTablaMercanciasConfigDatos).toHaveBeenCalledWith(mockEvent);
  });
  ...
  it('datasolicituActualizar should call updateDatosSolicitudFormState on the store', () => {
    ...
    expect(mockTramite260304Store.updateDatosSolicitudFormState).toHaveBeenCalledWith(mockEvent);
  });
  ...
  it('datosDeTablaSeleccionados should call update on the store with correct data', () => {
    ...
    expect(mockTramite260304Store.update).toHaveBeenCalled();
  });
