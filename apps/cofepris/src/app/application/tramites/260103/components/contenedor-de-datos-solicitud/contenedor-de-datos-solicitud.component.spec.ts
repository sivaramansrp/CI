...
describe('ContenedorDeDatosSolicitudComponent', () => {
  ...
  let mockTramite260103Query: jest.Mocked<Tramite260103Query>;
  let mockTramite260103Store: jest.Mocked<Tramite260103Store>;

  beforeEach(() => {
    mockTramite260103Query = {
      selectTramiteState$: of({
        opcionConfigDatos: [],
        scianConfigDatos: [],
        tablaMercanciasConfigDatos: [],
      }) as any, // Ensure compatibility with the expected type
    } as unknown as jest.Mocked<Tramite260103Query>; // Cast to jest.Mocked type

    mockTramite260103Store = {
    ...
    } as unknown as jest.Mocked<Tramite260103Store>; // Cast to jest.Mocked type

    TestBed.configureTestingModule({
      declarations: [],
      imports: [CommonModule, DatosDeLaSolicitudComponent, ContenedorDeDatosSolicitudComponent,HttpClientModule],
      providers: [
        { provide: Tramite260103Query, useValue: mockTramite260103Query },
        { provide: Tramite260103Store, useValue: mockTramite260103Store },
  ...
  it('opcionSeleccionado should call updateOpcionConfigDatos on the store', () => {
    ...
    expect(mockTramite260103Store.updateOpcionConfigDatos).toHaveBeenCalledWith(mockEvent);
  });
  ...
  it('scianSeleccionado should call updateScianConfigDatos on the store', () => {
    ...
    expect(mockTramite260103Store.updateScianConfigDatos).toHaveBeenCalledWith(mockEvent);
  });
  ...
  it('mercanciasSeleccionado should call updateTablaMercanciasConfigDatos on the store', () => {
    ...
    expect(mockTramite260103Store.updateTablaMercanciasConfigDatos).toHaveBeenCalledWith(mockEvent);
  });
  ...
  it('datasolicituActualizar should call updateDatosSolicitudFormState on the store', () => {
    ...
    expect(mockTramite260103Store.updateDatosSolicitudFormState).toHaveBeenCalledWith(mockEvent);
  });
  ...
  it('datosDeTablaSeleccionados should call update on the store with correct data', () => {
    ...
    expect(mockTramite260103Store.update).toHaveBeenCalled();
  });
