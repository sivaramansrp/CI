    ...
    return {
      ...
      solicitud: {
        ...
        informacionConfidencial:
          state.datosSolicitudFormState.publico === 'si' ? true : false,
      },
      establecimiento: {
      ...
      mercancias: (state.tablaMercanciasConfigDatos ?? []).map((mercancia) => {
        return {
          ...
          idClasificacionProducto:
            mercancia.claveClasificacionProductoObj?.clave,
          nombreClasificacionProducto:
            mercancia.claveClasificacionProductoObj?.descripcion,
          ideSubClasificacionProducto:
            mercancia.especificarClasificacionObj?.clave,
          nombreSubClasificacionProducto:
            mercancia.especificarClasificacionObj?.descripcion,
          ...
          nombreCortoPaisProcedencia:
            mercancia.paisDeProcedenciaDatos?.toString(),
      ...
      gridTerceros_TIPERS_FAB: state.fabricanteTablaDatos.map((fabricante) => {
        return {
          ...
          booleanExtranjero:
            fabricante.nacionalidad === 'Extranjero' ? '1' : '0',
          booleanFisicaNoContribuyente: '0',
          denominacion:
            fabricante.tipoPersona === 'Moral'
      ...
      gridTerceros_TIPERS_DES: state.destinatarioFinalTablaDatos.map(
        (destinatario) => {
          return {
            idPersonaSolicitud: '',
