...
export class GuardarAdapter_260212 {
  /**
   ...
   */
  static toFormPayload(state: Tramite260212State): unknown {
    return {
      "solicitante": {
        "rfc": "AAL0409235E6",
  ...
  private static mapScianData(arr?: any[]): any[] {
    ...
    /**
     * Map an API response (form payload) back into a partial Tramite260212State.
     ...
     * @returns Partial<Tramite260212State>
     */
    // eslint-disable-next-line complexity
    static fromApiResponse(response: unknown): Partial<Tramite260212State> {
        ...
        const PARTIAL: Partial<Tramite260212State> = {
     ...
     * @param store Optional Tramite260212Store instance to apply the patch
     * @returns Partial<Tramite260212State> (and patches the store when provided)
     */
    static patchToStore(response: any, store?: Tramite260212Store): Partial<Tramite260212State> {
        const PARTIAL = GuardarAdapter_260212.fromApiResponse(response);
        if (store) {
