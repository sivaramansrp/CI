...
export class DomicilioDelEstablecimiento260904Component
 ...
 /**
   ...
   */
    private _mercanciaFormState: MercanciaForm = DomicilioDelEstablecimiento260904Component.getEmptyMercanciaForm();
  ...
  onModificarMercancia(index: number): void {
    ...
    this.mercanciaFormState = DomicilioDelEstablecimiento260904Component.mapMercanciasInfoToForm(SELECTED_ROW);
  ...
  openMercanciaModal(): void {
    ...
    if (this.selectedMercanciaIndex === null) {
      ...
      const RESET_AND_OPEN = (): void => {
        this.mercanciaFormState = DomicilioDelEstablecimiento260904Component.getEmptyMercanciaForm();
        if (this.datosMercanciaContenedoraComp) {
          this.datosMercanciaContenedoraComp.crearMercanciaForm();
 ...
 /**
  ...
  constructor(
    ...
    private tramite260911Store: Tramite260904Store,
...
ngOnChanges(): void {
  ...
  setValorStore(FormGroup: FormGroup, control: string): void {
    ...
    this.tramite260911Store.setTramite260904State({
      [control]: VALOR
    });
  }
...
public markAllFieldsTouched(): void {
 if (this.form) {
  ...
  private resetRepresentanteLegalForm(state: Tramite260904State): void {
     ...
     *
     * @returns {boolean} `true` si la propiedad `mercanciasTablaDatos` existe y tiene al menos un elemento; de lo contrario, `false`.
     */
 ...
 *
 * @remarks
 * Puede ser utilizado para forzar la visualización de errores o para indicar que el usuario ha interactuado con la tabla de mercancías.
 */
