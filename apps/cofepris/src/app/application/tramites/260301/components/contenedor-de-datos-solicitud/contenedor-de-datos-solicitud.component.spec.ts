...
describe('ContenedorDeDatosSolicitudComponent', () => {
  ...
  let mockTramite260301Query: jest.Mocked<Tramite260301Query>;
  let mockTramite260301Store: jest.Mocked<Tramite260301Store>;

  beforeEach(() => {
    mockTramite260301Query = {
      selectTramiteState$: of({
        opcionConfigDatos: [],
        scianConfigDatos: [],
        tablaMercanciasConfigDatos: [],
      }) as any, // Ensure compatibility with the expected type
    } as unknown as jest.Mocked<Tramite260301Query>; // Cast to jest.Mocked type

    mockTramite260301Store = {
    ...
    } as unknown as jest.Mocked<Tramite260301Store>; // Cast to jest.Mocked type

    TestBed.configureTestingModule({
      declarations: [],
      imports: [CommonModule, DatosDeLaSolicitudComponent, ContenedorDeDatosSolicitudComponent,HttpClientModule],
      providers: [
        { provide: Tramite260301Query, useValue: mockTramite260301Query },
        { provide: Tramite260301Store, useValue: mockTramite260301Store },
  ...
  it('opcionSeleccionado should call updateOpcionConfigDatos on the store', () => {
    ...
    expect(mockTramite260301Store.updateOpcionConfigDatos).toHaveBeenCalledWith(mockEvent);
  });
  ...
  it('scianSeleccionado should call updateScianConfigDatos on the store', () => {
    ...
    expect(mockTramite260301Store.updateScianConfigDatos).toHaveBeenCalledWith(mockEvent);
  });
  ...
  it('mercanciasSeleccionado should call updateTablaMercanciasConfigDatos on the store', () => {
    ...
    expect(mockTramite260301Store.updateTablaMercanciasConfigDatos).toHaveBeenCalledWith(mockEvent);
  });
  ...
  it('datasolicituActualizar should call updateDatosSolicitudFormState on the store', () => {
    ...
    expect(mockTramite260301Store.updateDatosSolicitudFormState).toHaveBeenCalledWith(mockEvent);
  });
  ...
  it('datosDeTablaSeleccionados should call update on the store with correct data', () => {
    ...
    expect(mockTramite260301Store.update).toHaveBeenCalled();
  });
