...
export class PasoUnoComponent implements OnDestroy, OnInit {
   ...
   * Permite acceder a métodos y propiedades públicas del componente de terceros relacionados.
   */
   ...
   *
   * @param tramite260208Query Servicio para consultar el estado del trámite.
   * @param tramite260208Store Servicio para actualizar el estado del trámite.
      ...
      .pipe(
        ...
        map((seccionState) => {
          ...
          if (
            !(
              this.consultaState &&
              this.consultaState.procedureId === '260210' &&
  ...
  validarPasoUno(): boolean {
    ...
    const ES_TERCEROS_VALIDO =
      this.tercerosRelacionadosVistaComponent.validarContenedor() ?? false;
    return ES_TAB_VALIDO && ES_TERCEROS_VALIDO ? true : false;
  }
  ...
  getRegistroTomaMuestrasMercanciasData(): Observable<Tramite260208State> {
    return this.http.get<Tramite260208State>(
      'assets/json/260208/respuestaDeActualizacionDe.json'
    );
  }
  ...
  public getDatosSolicitudData(): void {
    this.datosSolicitudConsultaService
      .getDatosSolicitudData()
      .pipe(takeUntil(this.destroyNotifier$))
      .subscribe((resp) => {
        if (resp) {
