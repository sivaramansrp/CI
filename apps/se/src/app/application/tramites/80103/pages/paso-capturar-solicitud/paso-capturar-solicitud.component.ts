 ...
 */
import { AVISO, ConsultaioQuery, ConsultaioState, ERROR_FORMA_ALERT, JSONResponse, Usuario, WizardService, doDeepCopy, esValidObject, formatearFechaYyyyMmDd, getValidDatos } from '@ng-mf/data-access-user'
      ...
      this.nuevoProgramaIndustrialService.guardarDatosPost(PAYLOAD).subscribe(response => {
        ...
        if(esValidObject(DATOS) && esValidObject(DATOS.datos)) {
          if(getValidDatos(DATOS.datos.id_solicitud)) {
            this.tramite80103Store.setIdSolicitud(DATOS.datos.id_solicitud);
          } else {
