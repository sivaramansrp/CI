 ...
 * @providedIn root
 * @description
 * Decorador que marca la clase `RegistroDeSolicitudService` como un servicio inyectable en Angular.
 */
@Injectable({
  providedIn: 'root'
})
export class RegistroDeSolicitudService {
   ...
   */
  constructor(private http: HttpClient , private cancelacionStore: CancelacionStore) {
   ...
   */
  getImportacionDefinitivaData(): Observable<CancelacionState> {
    return this.http.get<CancelacionState>('assets/json/140102/registro-de-solicitud.json');
  }
  ...
  actualizarEstadoFormulario(campo: string, valor: unknown): void {
    this.cancelacionStore.setDynamicFieldValue(campo, valor);
  }
