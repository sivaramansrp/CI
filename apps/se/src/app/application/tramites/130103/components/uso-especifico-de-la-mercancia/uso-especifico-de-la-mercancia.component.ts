...
import { ConfiguracionColumna, ModeloDeFormaDinamica, TablaDinamicaComponent, TablaSeleccion } from '@libs/shared/data-access-user/src';
  ...
  */
  public obtenerFraccionArancelaria(): void {
    this.importacionDefinitivaService.getFraccionArancelaria().subscribe((resp) => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const FRACCION_FIELD: any = this.usoEspecificoFormData.find((datos: ModeloDeFormaDinamica) => datos.campo === 'fraccion_arancelaria');
      if (FRACCION_FIELD) {
