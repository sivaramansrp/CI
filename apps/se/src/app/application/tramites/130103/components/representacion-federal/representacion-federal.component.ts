...
/**
  ...
  */
  ngOnInit(): void {
    ...
    const ENTIDAD_FIELD: any = this.representacionFormdata.find((datos: ModeloDeFormaDinamica) => datos.campo === 'entidad');
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const REPRESENTACION_FIELD: any = this.representacionFormdata.find((datos: ModeloDeFormaDinamica) => datos.campo === 'reprsentation_federal');
    if (ENTIDAD_FIELD) {
      if (!ENTIDAD_FIELD.opciones && this.entidad) {
        ENTIDAD_FIELD.opciones = this.entidad.map((item: { id: number; descripcion: string }) => ({
          descripcion: item.descripcion,
          id: item.id,
        }));
      }
    ...
    if (REPRESENTACION_FIELD) {
      if (!REPRESENTACION_FIELD.opciones && this.representacion) {
        REPRESENTACION_FIELD.opciones = this.representacion.map((item: { id: number; descripcion: string }) => ({
          descripcion: item.descripcion,
          id: item.id,
        }));
      }
