...
import { ID_PROCEDIMIENTO, OPINIONES_SOLICITUD, PRODUCTO_OPCION } from '../../constants/exportacion-minerales-de-hierro-pasos.enum';
...
/*
  ...
  validarYEnviarFormulario(): void {
    if (this.partidasDelaMercanciaForm.invalid) {
      ...
      this.tramite130202Store.actualizarEstado({
  ...
  */
  ...
  setValoresStore($event: { form: FormGroup; campo: string }): void {
    ...
    this.tramite130202Store.actualizarEstado({ [$event.campo]: VALOR });
 ...
 */
  ...
  */
  getRegimenCatalogo(): void {
    this.exportacionMineralesDeHierroService.getRegimenCatalogo(this.idProcedimiento.toString()).subscribe((data) => {
   ...
   * @returns {void}
   */
  getClasificacionRegimenCatalogo(regimenId: string): void {
    this.exportacionMineralesDeHierroService.getClasificacionRegimenCatalogo(regimenId).subscribe((data) => {
  ...
  this.exportacionMineralesDeHierroService.getFraccionCatalogoService().subscribe((data) => {
   ...
   * @param FRACCION_ID 
   */
  getUnidadesMedidaTarifaria(fraccion: string): void {
    this.exportacionMineralesDeHierroService.getUMTService(fraccion).subscribe((data) => {
      ...
      if (this.unidadCatalogo.length > 0) {
        ...
        this.tramite130202Store.actualizarEstado({ unidadMedida: this.unidadCatalogo[0]?.clave || '' });
      }
    });
  }
   ...
   */
  getBloque(): void {
    this.exportacionMineralesDeHierroService.getBloqueService(this.idProcedimiento.toString()).subscribe((data) => {
  ...
  getPaisesPorBloque(ID: string): void {
    this.exportacionMineralesDeHierroService.getPaisesPorBloqueService(this.idProcedimiento.toString(), ID).subscribe((data) => {
  ...
  getEntidadesFederativasCatalogo(): void {
    this.exportacionMineralesDeHierroService.getEntidadesFederativasCatalogo(this.idProcedimiento.toString()).subscribe((data) => {
  ...
  this.exportacionMineralesDeHierroService.getRepresentacionFederalCatalogo(cveEntidad).subscribe((data) => {
  ...
  todosPaisesSeleccionados(evento: boolean): void {
    if (evento) {
      this.exportacionMineralesDeHierroService.getTodosPaisesSeleccionados(this.idProcedimiento.toString()).subscribe((data) => {
        this.paisesPorBloque = data as Catalogo[];
      });
    }
  }
  ...
  getMostrarPartidas(): void {
     ...
     this.exportacionMineralesDeHierroService.getMostrarPartidasService(idSolicitud).subscribe((data) => {
       if(data.codigo === '00'){
           ...
           if(this.mostrarPartidas.length > 0){
            const TABLE_BODY = this.mostrarPartidas.map((item, i) => ({
               ...
               this.tramite130202Store.actualizarEstado({tableBodyData: TABLE_BODY })
           }
           this.tramite130202Store.actualizarEstado({ mostrarPartidas: this.mostrarPartidas });
       }
     });
   }
  ...
  fechasSeleccionadas(evento: string[]): void {
    this.tramite130202Store.actualizarEstado({ fechasSeleccionadas: evento });
  }
