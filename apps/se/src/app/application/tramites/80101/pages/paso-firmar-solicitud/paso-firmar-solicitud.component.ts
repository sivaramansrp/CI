...
import { CadenaOriginalService } from '@libs/shared/data-access-user/src';
import { CategoriaMensaje } from '@libs/shared/data-access-user/src';
import { DocumentoService } from '@libs/shared/data-access-user/src';
...
import { TramiteFolioQueries } from '@libs/shared/data-access-user/src';
import { TramiteFolioStore } from '@libs/shared/data-access-user/src';
import { base64ToHex } from '@libs/shared/data-access-user/src';
import { encodeToISO88591Hex } from '@libs/shared/data-access-user/src';

/**
 * Componente Angular que representa el paso de firmar la solicitud dentro del flujo de trámites.
 * 
 * Este componente se utiliza para mostrar la interfaz de usuario correspondiente al paso de firma
 * de la solicitud en el proceso de trámites. La plantilla asociada define la estructura visual y 
 * los elementos interactivos necesarios para completar este paso.
 * 
 * @selector app-paso-firmar-solicitud - Selector utilizado para referenciar este componente en el HTML.
 * @templateUrl ./paso-firmar-solicitud.component.html - Ruta al archivo de plantilla HTML que define la vista del componente.
 */
@Component({
 ...
 */
  ...
  constructor(
    ...
    private tramite180101Query: Tramite80101Query,
  ...
  obtenerCadenaOriginal(): void {
    ...
    this.nuevoProgramaIndustrialService.obtenerCadenaOriginal(String(this.solicitudState.idSolicitud), PAYLOAD).subscribe({
      next: (resp) => {
        if (resp.codigo !== '00') {
    ...
    this.documentoService
      ...
      .pipe(
        ...
        switchMap((response) => {
          ...
          return this.nuevoProgramaIndustrialService.enviarFirma<string>(String(this.solicitudState.idSolicitud), PAYLOAD);
        }),
