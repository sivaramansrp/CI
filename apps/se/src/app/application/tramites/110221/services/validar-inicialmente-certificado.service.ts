...
import {
  ...
  Tramite110221Store,
} from '../../../estados/tramites/Tramite110221.store';
...
export class ValidarInicialmenteCertificadoService {
  ...
  constructor(
    ...
    public tramite110221Store: Tramite110221Store
  ) {}
  ...
  obtenerProductorPorExportador(): Observable<ProductorExportador> {
    return this.http.get<ProductorExportador>(
      'assets/json/110221/productor-exportador.json'
    );
  }
  ...
  obtenerMercancia(): Observable<MercanciasHistorico> {
    return this.http.get<MercanciasHistorico>(
      'assets/json/110221/mercancias-seleccionadas.json'
    );
  }
   ...
   * @returns Observable con los datos del estado de la solicitud `Solicitud110221State`,
   *          cargados desde el archivo JSON especificado en la ruta de `assets`.
   */
  getRegistroTomaMuestrasMercanciasData(): Observable<Solicitud110221State> {
    return this.http.get<Solicitud110221State>(
      'assets/json/110221/datos-prefill.json'
    );
  }
   ...
   * @param DATOS - Estado de la solicitud `Solicitud110221State` con la información
   *                del tipo de solicitud a actualizar en el store.
   */
  actualizarEstadoFormulario(DATOS: Solicitud110221State): void {
    this.tramite110221Store.update((state) => ({
      ...state,
      ...DATOS,
    }));
  }
