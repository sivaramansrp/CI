...
import { FRACCION_EXPORTACION, IMMEX_SERVICIO, NICO_TABLA, NicoInfo, fraccionInfo, immexInfo } from '../models/immex-ampliacion-sensibles.model';
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, TituloComponent, ReactiveFormsModule, TablaDinamicaComponent,NotificacionesComponent,CatalogoSelectComponent],
})
...
pagenuevaNotificacion:boolean=false;
 ...
 @ViewChild('mercanciaImportacionModal', { static: true }) mercanciaImportacionModal!: ElementRef;
modalInstance!: Modal;

  constructor(private fb: FormBuilder,public permisoImmexDatosService: PermisoImmexDatosService) {
    ...
    this.importacionForm = this.fb.group({
      ...
      umt: [''],
      descripcionTigie: [''],
      cantidadAnual: ['', [Validators.required, Validators.pattern('^[0-9]+$')]],
      capacidadInstalada: ['', [Validators.required, Validators.pattern('^[0-9]+$')]],
      cantidadPorPeriodo: ['', [Validators.required, Validators.pattern('^[0-9]+$')]],
      nicos: [''],
  ...
  this.modalInstance = new Modal(this.mercanciaImportacionModal.nativeElement, { backdrop: 'static' });
}
  ...
  onNumberInput(event: Event, formGroupName: string, controlName: string): void {
    ...
    if (formGroupName === 'exportacionForm') {
      this.exportacionForm.get(controlName)?.setValue(INPUT.value, { emitEvent: false });
    } else if (formGroupName === 'importacionForm') {
      this.importacionForm.get(controlName)?.setValue(INPUT.value, { emitEvent: false });
    }
  }

  // Check cantidadPorPeriodo ≤ cantidadAnual / 3
  checkCantidadPorPeriodo(): boolean {
    if(this.importacionForm.value.cantidadAnual === '' || this.importacionForm.value.cantidadPorPeriodo === '' || this.importacionForm.value.capacidadInstalada === ''){
        this.mostrarNotificacion('Los campos marcados con (*) son requeridos.');
      return false;
    }
    const CANTIDAD_ANUAL = Number(this.importacionForm.get('cantidadAnual')?.value);
    const CANTIDAD_POR_PERIODO = Number(this.importacionForm.get('cantidadPorPeriodo')?.value);
...
this.firstloadCompleted=false;
  if(this.importacionForm && this.importacionForm.value.fraccionArancelaria) {
     const EXISTS = this.immexTableDatos.some(
    (row) => row.fraccionArancelaria === this.importacionForm.value.fraccionArancelaria
  );
  if(!EXISTS){
    this.importacionForm.get('id')?.setValue(Math.floor(Math.random() * 1000));
    this.importacionForm.get('fraccionArancelaria')?.disable();
    this.importacionForm.get('umt')?.setValue('PZA', { emitEvent: false });
    this.importacionForm.get('descripcionTigie')?.setValue(
    ...
    this.pagemostrarNotificacion('Tiene que introducir la Fracción arancelaria.');
  }
}
  ...
  if(this.importacionForm.valid){
    if(this.checkCantidadPorPeriodo()){
      const NUEVO_REGISTRO: immexInfo = {
        id: Number(this.importacionForm.get('id')?.value)=== 0 ? Math.floor(Math.random() * 1000): Number(this.importacionForm.get('id')?.value),
        fraccionArancelaria: this.importacionForm.get('fraccionArancelaria')?.value,
...
if (INDEX > -1) {
 this.selectFraccionArancelaria = NUEVO_REGISTRO;
  this.immexTableDatos[INDEX] = { ...NUEVO_REGISTRO, numero: this.immexTableDatos[INDEX].numero };
...
this.importacionForm.get('id')?.setValue(0);
      ...
      this.nuevaNotificacion = {} as Notificacion;
        ...
        this.mostrarNotificacion('La capacidad instalada por periodo debe ser menor o igual a la cantidad anual.');
    }
  }
      this.mostrarNotificacion('Los campos marcados con (*) son requeridos.');
...
agregarExportacion():void{
  if(this.exportacionForm.getRawValue().descripcionComercialExport.trim() && this.exportacionForm.getRawValue().fraccionImportacion.trim()){
    if(this.selectFraccionArancelaria){
  if(this.selectFraccionArancelaria.fraccionArancelaria === this.exportacionForm.getRawValue().fraccionImportacion){
 const EXISTS = this.fraccionTablaDatos.some(
    (row) => row.descripcionComercialExport === this.exportacionForm.getRawValue().descripcionComercialExport?.toUpperCase() && row.fraccionExportacion === this.selectFraccionArancelaria.fraccionArancelaria
  );
  if(!EXISTS){
this.fraccionTablaDatos = [...this.fraccionTablaDatos,{
  id: Math.floor(Math.random() * 1000000) + 1,
  fraccionExportacion: this.selectFraccionArancelaria.fraccionArancelaria,
  fraccionImportacion: this.selectFraccionArancelaria.fraccionArancelaria,
  ...
  descripcionComercialExport: this.exportacionForm.value.descripcionComercialExport.toUpperCase(),
...
eliminarExportacion():void{
  if(this.selectExportacion && this.selectExportacion.id){
    this.fraccionTablaDatos = this.fraccionTablaDatos.filter(row => row.id !== this.selectExportacion.id);
...
mostrarDetalleMercancia():void{
  if(this.selectFraccionArancelaria && this.selectFraccionArancelaria.id){
    this.importacionForm.patchValue({
      ...
      capacidadInstalada: this.selectFraccionArancelaria.capacidadInstalada || '',
      cantidadPorPeriodo: this.selectFraccionArancelaria.cantidadPorPeriodo || '',
      nicos: this.selectFraccionArancelaria.nicos || '',
      productoDescExportacions: this.selectFraccionArancelaria.productoDescExportacions || '',
    });
    this.importacionForm.get('fraccionArancelaria')?.disable();
    this.importacionForm.get('umt')?.disable();
    this.importacionForm.get('descripcionTigie')?.disable();
...
descripcionNico():void{
  this.importacionForm.get('productoDescExportacions')?.setValue(`PRODUCTO-${Math.random().toString(36).substring(2, 15)}-${Date.now()}`);
}

agregarNico():void{
  if(this.importacionForm.value.nicos && this.importacionForm.getRawValue().productoDescExportacions){
  ...
  if(!EXISTS){
    this.nicoTablaDatos = [...this.nicoTablaDatos,{
      ...
      NICO_Columna_2: this.importacionForm.getRawValue().productoDescExportacions,
      estatus:false
  }]
  ...
  this.mostrarNotificacion('El NICO que intenta ingresar ya se encuentra registrado.');
}
  }
  else{
    this.mostrarNotificacion('Tiene que introducir el NICO y su descripción.');
  }
}
eliminarNico():void{
  if(this.selectedNicos && this.selectedNicos.length > 0){
    const IDS_TO_DELETE = this.selectedNicos.map(nico => nico.id);
    this.nicoTablaDatos = this.nicoTablaDatos.filter(nico => !IDS_TO_DELETE.includes(nico.id));
    this.selectedNicos = [];
  }
  else{
