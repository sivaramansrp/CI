...
import { Tramite240321State, Tramite240321Store } from './tramite240321Store.store';
import { Injectable } from '@angular/core';
import { Query } from '@datorama/akita';

/**
 * Servicio que permite consultar (leer) el estado del Trámite 240321
 * usando el patrón de Akita para manejo de estado.
 */

@Injectable({ providedIn: 'root' })
export class Tramite240321Query extends Query<Tramite240321State> {
  /**
   * Constructor que inicializa el query con el store correspondiente.
   *
   * @param {Tramite240321Store} store - Instancia del store para el Trámite 240321.
   */
  constructor(protected override store: Tramite240321Store) {
    super(store);
  }
   ...
   *
   * @property {Observable<Tramite240321State>} selectTramiteState$
   */
  ...
  /**
   * Observable que emite el folio del trámite.
   *
   * @property {Observable<string>} getFolio$
   */
  public getFolio$ = this.select((state) => state.folio);
}
