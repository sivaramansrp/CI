...
/**
 * Representa el estado de la solicitud para perfiles del formulario 32615.
 */
export interface Solicitud32615PerfilesState {
    /** actualizacion está documentado. */
    actualizacion: string;

  /** ¿Existe un procedimiento documentado? */
  procedimientoDocumentado: string;

  /** Indique el número del indiqueNumero. */
  ...
  /** Resultados obtenidos de las auditorías o evaluaciones. */
  resultados: string;

  /** Indique si el enfoque de las auditorías es en base al tamaño de la organización y a la naturaleza de los riesgos. */
  naturaleza: string;

  /** Detalle la ubicación de los elementos de seguridad. */
  detalleSuUbicacion: string;

  /** Describa Detalle la ubicación de los elementos de seguridad. */
  describaDetalleSuUbicacion: string;
  /** Describa el mantenimiento realizado a los elementos de seguridad. */
  describaMantenimiento: string;

  /** Mencionar el procedimiento documentado en el que indique el funcionamiento del sistema central de alarmas externo o sensores. */
  procedimientoDocumentadoMencionar: string;
    /** encargado */
  encargado:string
    /** Verificar si la información es correcta. */
  verificarCorrecta: string;
  /** Indique si la información es restringida. */
  restringido: string;
  /** Describa el procedimiento para las entregas de mensajería. */
  entregasMensajeria: string;
    /** Indique si se cuenta con un recinto fiscalizado. */
  recintoFiscalizado: string;
}

/**
 * Crea el estado inicial del formulario `Solicitud32615PerfilesState`.
 * Todos los campos se inicializan como cadenas vacías.
 *
 * @returns El estado inicial con todos los campos del formulario vacíos.
 */
export function createInitialState(): Solicitud32615PerfilesState {
  return {
    /** actualizacion está documentado. */
    actualizacion: '',
    ...
    /** Resultados obtenidos de las auditorías o evaluaciones realizadas. */
    resultados: '',
    /** Indique si el enfoque de las auditorías es en base al tamaño de la organización y a la naturaleza de los riesgos. */
    naturaleza: '',
    /** Detalle la ubicación de los elementos de seguridad. */
    detalleSuUbicacion: '',
    /** Describa Detalle la ubicación de los elementos de seguridad. */
    describaDetalleSuUbicacion: '',
    /** Describa el mantenimiento realizado a los elementos de seguridad. */
    describaMantenimiento: '',
    /** Mencionar el procedimiento documentado en el que indique el funcionamiento del sistema central de alarmas externo o sensores. */
    procedimientoDocumentadoMencionar: '',
    /** encargado */
    encargado: '', 
    /** Verificar si la información es correcta. */
    verificarCorrecta: '',
...
/**
 * Servicio que gestiona el estado de la solicitud 32615 para los perfiles.
 ...
 * @StoreConfig: Configura el almacenamiento del estado para la solicitud 32615.
 * El nombre del estado es `tramite32615Perfiles` y se establece como reseteable.
 */
@Injectable({
  providedIn: 'root', // Proporciona el servicio a nivel de toda la aplicación.
})
@StoreConfig({ name: 'tramite32615Perfiles', resettable: true }) // Configuración del estado en la tienda
export class Tramite32615PerfilesStore extends Store<Solicitud32615PerfilesState> {
  /**
  ...
  constructor() {
    ...
    /**
     * Método para actualizar el valor del campo `actualizacion` en el estado.
     * 
     * @param actualizacion: El nuevo valor para el campo `actualizacion`.
     * Este valor se asigna al estado utilizando la función `update()`, que actualiza solo el campo correspondiente.
     */
  public setActualizacion(actualizacion: string): void {
    this.update((state) => ({
      ...state, // Se copia el estado anterior
      actualizacion, // Se actualiza solo el campo `actualizacion`
    }));
  }
...
public setProgramacion(programacion: string): void {
  ...
  /**
   * Método para actualizar el valor del campo `resultados` en el estado.
   * 
   * @param resultados: El nuevo valor para el campo `resultados`.
   * Este valor se asigna al estado utilizando la función `update()`.
   */
  public setResultados(resultados: string): void {
    this.update((state) => ({
      ...state, // Se copia el estado anterior
      resultados, // Se actualiza solo el campo `resultados`
    }));
  }
  /**
   * Método para actualizar el valor del campo `naturaleza` en el estado.
   * 
   * @param naturaleza: El nuevo valor para el campo `naturaleza`.
   * Este valor se asigna al estado utilizando la función `update()`.
   */
  public setNaturaleza(naturaleza: string): void {
    this.update((state) => ({
      ...state, // Se copia el estado anterior
      naturaleza, // Se actualiza solo el campo `naturaleza`
    }));
  }
    /**
     * Método para actualizar el valor del campo `detalleSuUbicacion` en el estado.
     * 
     * @param detalleSuUbicacion: El nuevo valor para el campo `detalleSuUbicacion`.
     * Este valor se asigna al estado utilizando la función `update()`.
     */
  public setDetalleSuUbicacion(detalleSuUbicacion: string): void {
    this.update((state) => ({
      ...state, // Se copia el estado anterior
      detalleSuUbicacion, // Se actualiza solo el campo `detalleSuUbicacion`
    }));
  }
   /**
     * Método para actualizar el valor del campo `describaDetalleSuUbicacion` en el estado.
     * 
     * @param describaDetalleSuUbicacion: El nuevo valor para el campo `describaDetalleSuUbicacion`.
     * Este valor se asigna al estado utilizando la función `update()`.
     */
  public setDescribaDetalleSuUbicacion(describaDetalleSuUbicacion: string): void {
    this.update((state) => ({
      ...state, // Se copia el estado anterior
      describaDetalleSuUbicacion, // Se actualiza solo el campo `describaDetalleSuUbicacion`
    }));
  }
    /**
     * Método para actualizar el valor del campo `describaMantenimiento` en el estado.
     * 
     * @param describaMantenimiento: El nuevo valor para el campo `describaMantenimiento`.
     * Este valor se asigna al estado utilizando la función `update()`.
     */
  public setDescribaMantenimiento(describaMantenimiento: string): void {
    this.update((state) => ({
      ...state, // Se copia el estado anterior
      describaMantenimiento, // Se actualiza solo el campo `describaMantenimiento`
    }));
  }
    /**
     * Método para actualizar el valor del campo `procedimientoDocumentadoMencionar` en el estado.
     * 
     * @param procedimientoDocumentadoMencionar: El nuevo valor para el campo `procedimientoDocumentadoMencionar`.
     * Este valor se asigna al estado utilizando la función `update()`.
     */
  public setProcedimientoDocumentadoMencionar(procedimientoDocumentadoMencionar: string): void {
    this.update((state) => ({
      ...state, // Se copia el estado anterior
      procedimientoDocumentadoMencionar, // Se actualiza solo el campo `procedimientoDocumentadoMencionar`
    }));
  }
    /**
     * Método para actualizar el valor del campo `encargado` en el estado.
     * 
     * @param encargado: El nuevo valor para el campo `encargado`.
     * Este valor se asigna al estado utilizando la función `update()`.
     */
  public setEncargado(encargado: string): void {
    this.update((state) => ({
      ...state, // Se copia el estado anterior
      encargado, // Se actualiza solo el campo `encargado`
    }));
  }
    /**
     * Método para actualizar el valor del campo `verificarCorrecta` en el estado.
     * 
     * @param verificarCorrecta: El nuevo valor para el campo `verificarCorrecta`.
     * Este valor se asigna al estado utilizando la función `update()`.
     */
    public setVerificarCorrecta(verificarCorrecta: string): void {
        this.update((state) => ({
        ...state, // Se copia el estado anterior
        verificarCorrecta, // Se actualiza solo el campo `verificarCorrecta`
        }));
    }
    /**
     * Método para actualizar el valor del campo `restringido` en el estado.
     * 
     * @param restringido: El nuevo valor para el campo `restringido`.
     * Este valor se asigna al estado utilizando la función `update()`.
     */
    public setRestringido(restringido: string): void {
        this.update((state) => ({
            ...state, // Se copia el estado anterior
            restringido, // Se actualiza solo el campo `restringido`
        }));
    }
    /**
     * Método para actualizar el valor del campo `entregasMensajeria` en el estado.
     * 
     * @param entregasMensajeria: El nuevo valor para el campo `entregasMensajeria`.
     * Este valor se asigna al estado utilizando la función `update()`.
     */
    public setEntregasMensajeria(entregasMensajeria: string): void {
        this.update((state) => ({
            ...state, // Se copia el estado anterior
            entregasMensajeria, // Se actualiza solo el campo `entregasMensajeria`
        }));
    }
    /**
     * Método para actualizar el valor del campo `recintoFiscalizado` en el estado.
     * 
    ...
    public setRecintoFiscalizado(recintoFiscalizado: string): void {
        this.update((state) => ({
            ...state, // Se copia el estado anterior
            recintoFiscalizado, // Se actualiza solo el campo `recintoFiscalizado`
        }));
    }
}
