...
describe('DatosComunesComponent', () => {
  ...
  it('debería configurar el estado de solo lectura desde consultaioQuery', () => {
    ...
    expect(component.forma.get('cumplimientoFiscalAduanero')?.disabled).toBe(
      true
    );
    ...
    expect(component.forma.get('cumplimientoFiscalAduanero')?.enabled).toBe(
      true
    );
  ...
  it('debería aplicar validaciones requeridas a los controles específicos', () => {
    const cumplimientoControl = component.forma.get(
      'cumplimientoFiscalAduanero'
    );
  ...
  it('debería resetear el campo opuesto cuando se selecciona un valor', () => {
    ...
    component.manejarCamposMutuamenteExcluyentes(
      'sectorProductivo',
      'sectorServicio',
      mockEvent
    );
  ...
  it('debería marcar el campo opuesto como pristine', () => {
    ...
    if (sectorServicioControl) {
      const markAsPristineSpy = jest.spyOn(
        sectorServicioControl,
        'markAsPristine'
      );
  ...
  it('debería cargar las listas de sectores y bimestres', () => {
    ...
    expect(
      mockOeaTextilRegistroService.sectorListaDeSelects
    ).toHaveBeenCalled();
    expect(component.sectorProductivoList).toEqual(
      mockCatalogos.sectorProductivoList
    );
  ...
  it('debería configurar notificación con mensaje por defecto', () => {
    ...
    expect(component.nuevaNotificacion.mensaje).toBe(
      component.REQUISITO_OBLIGATORIO
    );
  });
  ...
  it('debería habilitar diálogo cuando selección es verdadera', () => {
    ...
    component.enviarDialogData();
    expect(component.esHabilitarElDialogo).toBe(false);
    expect(component.enviarDialogData).toHaveBeenCalled();
  });
