 ...
 */
  ...
  /**
   * Maneja el evento keypress para los campos numéricos (cantidad, peso).
  ...
  onKeyPress(event: KeyboardEvent): boolean {
    ...
    // Para campos cantidad y peso: aplicar validaciones numéricas con límites
    if (INPUT?.id === 'cantidad' || INPUT?.id === 'peso') {
   ...
   * @description Método que valida y formatea números decimales para Cantidad y Peso.
   * - Formato: MÁXIMO 13 dígitos enteros + punto decimal + MÁXIMO 2 decimales (total 16 caracteres)
   * - BLOQUEA entrada después de 13 dígitos enteros o 2 decimales
