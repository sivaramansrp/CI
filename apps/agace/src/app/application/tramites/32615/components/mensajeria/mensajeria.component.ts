...
import { Tramite32615MensajeriaQuery } from '../../../../estados/queries/tramite32615_mensajeria.query';
import productivo from '@libs/shared/theme/assets/json/32615/productivo.json';
/**
 * Componente que gestiona la sección de mensajería para el trámite 32615.
 ...
 */
  ...
  public solicitudState!: Solicitud32615MensajeriaState;
  ...
  /** Bandera que indica si es "Comercio Exterior" */
  public isComercioExterior:boolean = false;

  /** Bandera que indica si es "Comercio Exterior" */
  public isRecintoFiscalizado:boolean = false;
  ...
  public fechaInicioInput: InputFecha = FECHA_DE_INICIO;

  /** Configuración del input de fecha de vigencia */
  public fechaVigenciaInput: InputFecha = FECHA_DE_VIGENCIA;
   ...
   * @param tramite32615Store Store del trámite 32615
   * @param tramite32615Query Query del trámite 32615
   */
  constructor(
  ...
  public inicializarEstadoFormulario(): void {
    ...
    if (this.esFormularioSoloLectura) {
      Object.keys(this.susFilialesForm.controls).forEach((key) => {
        this.susFilialesForm.get(key)?.disable();
      });
    } else {
      Object.keys(this.susFilialesForm.controls).forEach((key) => {
        this.susFilialesForm.get(key)?.enable();
      });
    }

    if (this.esFormularioSoloLectura) {
      Object.keys(this.lasEmpresasForm.controls).forEach((key) => {
        this.lasEmpresasForm.get(key)?.disable();
      });
    } else {
      Object.keys(this.lasEmpresasForm.controls).forEach((key) => {
        this.lasEmpresasForm.get(key)?.enable();
      });
    }
  }
  ...
  public crearFormulario(): void {
    this.mensajeriaGroup = this.fb.group({
      ...
      cancelacionRecintoFiscalizado: [this.solicitudState?.cancelacionRecintoFiscalizado],
      perfilRecintoFiscalizado: [this.solicitudState?.perfilRecintoFiscalizado],
      numeroAutorizacion: [this.solicitudState?.numeroAutorizacion],
      ...
      fechaInicioComercioExterior: [this.solicitudState?.fechaInicioComercioExterior],
      fechaVigencia: [this.solicitudState?.fechaVigencia],
    ...
    this.isComercioExterior = parseInt(this.solicitudState?.comercioExterior, 10) === 1;
    this.isRecintoFiscalizado = parseInt(this.solicitudState?.recintoFiscalizado, 10) === 1;
  }
  ...
  public setValoresStore(form: FormGroup, campo: string, metodoNombre: keyof Tramite32615MensajeriaStore): void {
    ...
    if (campo === 'comercioExterior' && parseInt(VALOR, 10) === 1) {
      this.isComercioExterior = true;
    } else if (campo === 'comercioExterior' && parseInt(VALOR, 10) === 0) {
      this.isComercioExterior = false;
    }
    if (campo === 'recintoFiscalizado' && parseInt(VALOR, 10) === 1) {
      this.isRecintoFiscalizado = true;
    } else if (campo === 'recintoFiscalizado' && parseInt(VALOR, 10) === 0) {
      this.isRecintoFiscalizado = false;
    }
  }
  ...
  public cambioFechaFactura(
    ...
    form: FormGroup,
    campo: string,
    metodoNombre: keyof Tramite32615MensajeriaStore
  ): void {
  ...
  public cambioFechaInicio(
    ...
    this.mensajeriaGroup.get(campo)?.setValue(nuevo_valor);
    this.mensajeriaGroup.get(campo)?.markAsUntouched();
    const VALOR = form.get(campo)?.value;
    (this.tramite32615Store[metodoNombre] as (value: unknown) => void)(VALOR);
  }
  /**
