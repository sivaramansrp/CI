...
import { Solicitud32610State } from '../../estados/solicitud32610.store';
...
/**
 * Componente para agregar y gestionar transportistas en el trámite 32610.
...
export class AgregarTransportistasComponent implements OnInit, OnDestroy {
   ...
   */
  @ViewChild('templateTransposrtistas')
  templateTransposrtistas!: TemplateRef<void>;
   ...
   * param tramite32610Store - Store para gestionar el estado del trámite
   * param tramite32610Query - Query para consultar el estado del trámite
...
obtenerEstadoSolicitud(): void {
  ...
  abrirModal1(templateTransposrtistas: TemplateRef<void>): void {
    this.modalRefabir = this.modalService.show(templateTransposrtistas, {
      class: 'modal-lg',
    });
  }
   ...
   */
  buscarRFC(): void {
    const RFC = this.transportistaCertificacionForm.get(
      'rfcEnclaveOperativo'
    )?.value;
    if (RFC) {
  ...
  existeRFCEnTabla(rfc: string): boolean {
    return this.transportistasLista.some(
      (empresa) =>
  ...
  mostrarModalRFCDuplicado(): void {
    ...
    this.modalRefabir = this.modalService.show(
  ...
  buscarDatosPorRFC(rfc: string): void {
    this.solicitudService
      .conseguirTransportistasLista(rfc)
  ...
  aceptarTransportista(): void {
    const RFC_VALUE = this.transportistaCertificacionForm.get(
      'rfcEnclaveOperativo'
    )?.value;

    // Verificar que el campo RFC requerido esté lleno
    if (!RFC_VALUE?.trim()) {
      this.transportistaCertificacionForm
    ...
    const DENOMINACION = this.transportistaCertificacionForm.get(
      'denominacionRazonsocial'
    )?.value;
    const DOMICILIO =
      this.transportistaCertificacionForm.get('domicilio')?.value;
    const ENLACE_RFC =
      this.transportistaCertificacionForm.get('enlaceOperativorfc')?.value;
    ...
    if (
      ...
      this.existeRFCEnTablaExcluyendo(
        RFC_VALUE,
        this.selectedTransportista?.rfcEnclaveOperativo
      )
    ) {
      this.mostrarModalDatosObligatorios();
      return;
    ...
    if (this.isEditMode && this.selectedTransportista) {
      ...
      const INDEX = this.transportistasLista.findIndex(
        (emp) =>
          emp.rfcEnclaveOperativo ===
          this.selectedTransportista?.rfcEnclaveOperativo
      );
      if (INDEX !== -1) {
  ...
  mostrarModalExito(): void {
    ...
    this.modalRefExito = this.modalService.show(
      this.templateExito,
      MODAL_CONFIG
    );
  }
  ...
  mostrarModalDatosObligatorios(): void {
    ...
    this.modalRefDatosObligatorios = this.modalService.show(
  ...
  mostrarModalDatos(): void {
    ...
    this.modalRefDatos = this.modalService.show(
      this.templateDatos,
      MODAL_CONFIG
    );
  }
  ...
  if (!this.selectedTransportista) {
    ...
    this.transportistaCertificacionForm.patchValue({
      ...
      denominacionRazonsocial:
        this.selectedTransportista.denominacionRazonsocial,
  ...
  confirmarEliminacionTransportista(): void {
    ...
    this.transportistasLista = this.transportistasLista.filter(
      (empresa) =>
  ...
  actualizarTransportistasListaEnStore(): void {
    this.tramite32610Store.actualizarEstado({
      transportistasLista: this.transportistasLista,
    });
  }
