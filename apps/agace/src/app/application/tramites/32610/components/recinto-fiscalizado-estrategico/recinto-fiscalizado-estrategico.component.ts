 ...
 */
@Component({
  selector: 'app-recinto-fiscalizado-estrategico',
  standalone: true,
  ...
  templateUrl: './recinto-fiscalizado-estrategico.component.html',
  styleUrl: './recinto-fiscalizado-estrategico.component.scss',
})
export class RecintoFiscalizadoEstrategicoComponent implements OnInit, OnDestroy {
  /**
   ...
   */
  recintoFiscalizadoEstrategicoForm!: FormGroup;
  ...
  /**
   * Estado actual de la solicitud del trámite 32610.
   ...
   * Importado desde las constantes de datos comunes.
   */
  empresaGrupoPanels = EMPRESA_DE_GRUPO_PANELS;

  /**
   * Configuración de paneles colapsables secundarios para la interfaz.
   * Importado desde las constantes de datos comunes.
   */
  transportistasPanels = TRANSPORTISTAS_PANELS;
   ...
   */
  @ViewChild('templateDatosObligatorios')
  templateDatosObligatorios!: TemplateRef<void>;

  /**
   * Referencia al template del modal de confirmación de eliminación.
   * Template para confirmar la eliminación de empresas.
   */
  @ViewChild('templateConfirmacionEliminacion')
  templateConfirmacionEliminacion!: TemplateRef<void>;

  /**
   * Referencia al template del modal de selección requerida.
   * Se muestra cuando se requiere seleccionar un elemento de la tabla.
   */
  @ViewChild('templateSeleccionRequerida')
  templateSeleccionRequerida!: TemplateRef<void>;

  /**
   * Referencia al componente de transportistas para validación
   */
  @ViewChild('agregarTransportistasRef')
  componenteAgregarTransportistas!: AgregarTransportistasComponent;
  /**
  ...
  guardarDatosFormulario(): void {
    this.inicializarFormulario();
    if (this.esFormularioSoloLectura) {
      this.recintoFiscalizadoEstrategicoForm.disable();
    } else {
      this.recintoFiscalizadoEstrategicoForm.enable();
    }
  }
  ...
  inicializarFormulario(): void {
    ...
    this.recintoFiscalizadoEstrategicoForm = this.fb.group({
      comercioExteriorRealizado: [
        this.solicitudState?.comercioExteriorRealizado,
        Validators.required,
      ],
      ...
      esParteGrupoComercioExterior: [
        this.solicitudState?.esParteGrupoComercioExterior,
      ],
      fusionEscisionConOperacionExterior: [
        this.solicitudState?.fusionEscisionConOperacionExterior,
      ],
      ...
      registroEsquemaCertificacion: [
        this.solicitudState?.registroEsquemaCertificacion,
        Validators.required,
      ],
      tipoInformacionEmpresa: [
        this.solicitudState?.tipoInformacionEmpresa,
        Validators.required,
      ],
    });
    this.agregarEnlaceOperativoForm = this.fb.group({
      rfcEnclaveOperativo: [
        this.solicitudState?.rfcEnclaveOperativo,
        Validators.required,
      ],
      enlaceOperativorfc: [
      ...
      inputfechaDeLaUltimaOperacion: [
        {
          value: this.solicitudState?.inputfechaDeLaUltimaOperacion,
          disabled: true,
        },
      ],
    });
  ...
  onFechaCambiada(nuevo_valor: string): void {
    this.recintoFiscalizadoEstrategicoForm.get('fechaDePago')?.setValue(nuevo_valor);
    this.recintoFiscalizadoEstrategicoForm.get('fechaDePago')?.markAsUntouched();

    if (RecintoFiscalizadoEstrategicoComponent.esFechaFutura(nuevo_valor)) {
  ...
  actualizarFechaInicioComercio(nuevo_valor: string): void {
    this.recintoFiscalizadoEstrategicoForm
      .get('fechaInicioComercio')
      ?.setValue(nuevo_valor);
    this.recintoFiscalizadoEstrategicoForm.get('fechaInicioComercio')?.markAsUntouched();
  }
  ...
  actualizarFechaDeLaUltimaOperacion(nuevo_valor: string): void {
    this.agregarEnlaceOperativoForm
      .get('inputfechaDeLaUltimaOperacion')
      ?.setValue(nuevo_valor);
    this.agregarEnlaceOperativoForm
      .get('inputfechaDeLaUltimaOperacion')
      ?.markAsUntouched();
   ...
   */
  buscarRFC(): void {
    const RFC = this.agregarEnlaceOperativoForm.get(
      'rfcEnclaveOperativo'
    )?.value;
    if (RFC) {
  ...
  aceptarEnlaceOperativo(): void {
    const RFC_VALUE = this.agregarEnlaceOperativoForm.get(
      'rfcEnclaveOperativo'
    )?.value;
    const DATE_VALUE = this.agregarEnlaceOperativoForm.get(
      'inputfechaDeLaUltimaOperacion'
    )?.value;
    ...
    if (
      this.isEditMode &&
      this.existeRFCEnTablaExcluyendo(
        RFC_VALUE,
        this.selectedEmpresa?.rfcEnclaveOperativo
      )
    ) {
    ...
    if (DATE_VALUE && RecintoFiscalizadoEstrategicoComponent.esFechaFutura(DATE_VALUE)) {
      this.mostrarModalFechaInvalida();
      return;
    }
    ...
    if (this.isEditMode && this.selectedEmpresa) {
      const INDEX = this.tablaDatos.findIndex(
        (emp) =>
  ...
  mostrarModalDatosObligatorios(): void {
    ...
    this.modalRef = this.modalService.show(
      this.templateDatosObligatorios,
      MODAL_CONFIG
    );
  }
  ...
  modificarEmpresa(): void {
    ...
    this.agregarEnlaceOperativoForm.patchValue({
      ...
      inputfechaDeLaUltimaOperacion:
        this.selectedEmpresa.inputfechaDeLaUltimaOperacion || '',
    });
  ...
  mostrarModalSeleccionRequerida(): void {
    ...
    this.modalRef = this.modalService.show(
      this.templateSeleccionRequerida,
      MODAL_CONFIG
    );
  }
  ...
  mostrarModalConfirmacionEliminacion(): void {
    ...
    this.modalRef = this.modalService.show(
      this.templateConfirmacionEliminacion,
      MODAL_CONFIG
    );
  }
  ...
  mostrar_colapsable(index: number): void {
    const IS_CURRENTLY_OPEN = this.empresaGrupoPanels[index].isCollapsed;
    this.empresaGrupoPanels.forEach((panel, i) => {
  ...
  validarCamposPago(): void {
    const FECHADEPAGE = this.recintoFiscalizadoEstrategicoForm.get('fechaDePago')?.value;
    const MONTO = this.recintoFiscalizadoEstrategicoForm.get('monto')?.value;
    const OPERACIONESBANCARIAS = this.recintoFiscalizadoEstrategicoForm.get(
      'operacionesBancarias'
    )?.value;
    const LLAVEPAGO = this.recintoFiscalizadoEstrategicoForm.get('llavePago')?.value;
    ...
    if (CAMPOS_VACIOS) {
      this.recintoFiscalizadoEstrategicoForm.get('fechaDePago')?.markAsTouched();
      this.recintoFiscalizadoEstrategicoForm.get('monto')?.markAsTouched();
      this.recintoFiscalizadoEstrategicoForm
        .get('operacionesBancarias')
        ?.markAsTouched();
      this.recintoFiscalizadoEstrategicoForm.get('llavePago')?.markAsTouched();
    }
  }
  ...
  soloNumeros(event: KeyboardEvent): boolean {
    ...
    if (!this.recintoFiscalizadoEstrategicoForm) {
      return false;
    }
  ...
  pegarSoloNumeros(event: ClipboardEvent, fieldName: string): void {
    ...
    const LONGITUD_MAXIMA = parseInt(
      ELEMENTO_DESTINO.getAttribute('maxlength') || '10',
      10
    );
    const NUEVO_VALOR = SOLO_NUMEROS.substring(0, LONGITUD_MAXIMA);

    // Actualizar el control del formulario
    this.recintoFiscalizadoEstrategicoForm.get(fieldName)?.setValue(NUEVO_VALOR);
  }
  ...
  soloAlfanumericosConEspacios(event: KeyboardEvent): boolean {
    ...
    if (
      [8, 9, 27, 13, 46, 32].indexOf(CHARCODE) !== -1 ||
      // Permitir: Ctrl+A (65), Ctrl+C (67), Ctrl+V (86), Ctrl+X (88)
      (CHARCODE === 65 && event.ctrlKey === true) ||
      (CHARCODE === 67 && event.ctrlKey === true) ||
      (CHARCODE === 86 && event.ctrlKey === true) ||
  ...
  pegarSoloAlfanumericos(event: ClipboardEvent, fieldName: string): void {
    ...
    const ELEMENTO_DESTINO = event.target as HTMLInputElement;
    const LONGITUD_MAXIMA = parseInt(
      ELEMENTO_DESTINO.getAttribute('maxlength') || '25',
      10
    );
  ...
  actualizarTablaDatosEnStore(): void {
    this.tramite32610Store.actualizarEstado({ tablaDatos: this.tablaDatos });
  }
  ...
  debeValidarTabla(): boolean {
    const ES_PARTE_GRUPO = this.recintoFiscalizadoEstrategicoForm.get(
      'esParteGrupoComercioExterior'
    )?.value;
  ...
  /**
   * Verifica si el formulario `recintoFiscalizadoEstrategicoForm` es válido.
  ...
  validarFormulario(): boolean {
    ...
    // Validate main form
    if (this.recintoFiscalizadoEstrategicoForm.invalid) {
      this.recintoFiscalizadoEstrategicoForm.markAllAsTouched();
      esValido = false;
    }
    ...
    if (
      this.componenteAgregarTransportistas &&
      !this.componenteAgregarTransportistas.validarTransportistas()
    ) {
      esValido = false;
    }
    return esValido;
  ...
  configurarValidacionDinamica(): void {
    const COMERCIO_EXTERIORCONTROL = this.recintoFiscalizadoEstrategicoForm.get(
      'comercioExteriorRealizado'
    );

    if (COMERCIO_EXTERIORCONTROL) {
      // Escuchar cambios en comercioExteriorRealizado
      COMERCIO_EXTERIORCONTROL.valueChanges
        .pipe(takeUntil(this.destroy$))
        .subscribe((value) => {
      ...
      this.actualizarValidacionBasadaEnComercioExterior(
        COMERCIO_EXTERIORCONTROL.value
      );
    }
  }
  ...
  actualizarValidacionBasadaEnComercioExterior(value: string): void {
    const FECHA_INICIO_CONTROL = this.recintoFiscalizadoEstrategicoForm.get(
      'fechaInicioComercio'
    );
    const ES_PARTE_GRUPO_CONTROL = this.recintoFiscalizadoEstrategicoForm.get(
      'esParteGrupoComercioExterior'
    );
    const FUSION_ESCISION_CONTROL = this.recintoFiscalizadoEstrategicoForm.get(
      'fusionEscisionConOperacionExterior'
    );
    const EMPRESA_EXTRANJERA_CONTROL = this.recintoFiscalizadoEstrategicoForm.get(
