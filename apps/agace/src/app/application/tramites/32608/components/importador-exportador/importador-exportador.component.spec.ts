...
} from '../../enums/oea-textil-registro.enum';
import { RFCEnlaceOperativo, EmpresaDelGrupo } from '../../models/oea-textil-registro.model';
import { ComponentFixture, TestBed } from '@angular/core/testing';
import { FormBuilder, ReactiveFormsModule } from '@angular/forms';
import { BsModalRef, BsModalService } from 'ngx-bootstrap/modal';
import { of, throwError } from 'rxjs';
...
describe('Gestión del estado del formulario', () => {
  ...
  it('debería inicializar rubroIVATextilForm correctamente', () => {
    component.inicializarFormulario();
    
    expect(component.rubroIVATextilForm).toBeDefined();
    expect(component.rubroIVATextilForm.get('rubroCertificacion')).toBeTruthy();
    expect(component.rubroIVATextilForm.get('numeroOficio')).toBeTruthy();
    expect(component.rubroIVATextilForm.get('fechaFinVigenciaRubro')).toBeTruthy();
  });

  it('debería cargar datos del rubro textil desde el estado', () => {
    component.inicializarFormulario();
    
    expect(component.rubroIVATextilForm.get('rubroCertificacion')?.value)
      .toBe(mockSolicitudState.rubroCertificacion);
    expect(component.rubroIVATextilForm.get('numeroOficio')?.value)
      .toBe(mockSolicitudState.numeroOficio);
    expect(component.rubroIVATextilForm.get('fechaFinVigenciaRubro')?.value)
      .toBe(mockSolicitudState.fechaFinVigenciaRubro);
  });

  it('debería deshabilitar campos del rubro IVA en modo solo lectura', () => {
    component.inicializarFormulario();
    component.esFormularioSoloLectura = true;
    component.guardarDatosFormulario();
    
    expect(component.rubroIVATextilForm.disabled).toBe(true);
  });
});

describe('Registro esquema certificación IVA/IEPS', () => {
  beforeEach(() => {
    component.inicializarFormulario();
    fixture.detectChanges();
  });

  it('debería limpiar campos cuando se selecciona "No" (0)', () => {
    // Primero establecer valores
    component.rubroIVATextilForm.patchValue({
      rubroCertificacion: 'ABC',
      numeroOficio: '123',
      fechaFinVigenciaRubro: '01/01/2024'
    });

    component.onRegistroEsquemaCertificacionIVAIEPSChange('0');

    expect(component.rubroIVATextilForm.get('rubroCertificacion')?.value).toBe('');
    expect(component.rubroIVATextilForm.get('numeroOficio')?.value).toBe('');
    expect(component.rubroIVATextilForm.get('fechaFinVigenciaRubro')?.value).toBe('');
    expect(component.rubroIVATextilForm.enabled).toBe(true);
  });

  it('debería cargar datos desde servicio cuando se selecciona "Sí" (1)', () => {
    const spyGetDatos = jest.spyOn(mockOeaTextilRegistroService, 'getDatosrubroTextil');
    
    component.onRegistroEsquemaCertificacionIVAIEPSChange('1');

    expect(spyGetDatos).toHaveBeenCalled();
    expect(component.rubroIVATextilForm.disabled).toBe(true);
  });

  it('debería manejar errores al cargar datos del rubro textil', () => {
    const consoleSpy = jest.spyOn(console, 'error').mockImplementation();
    mockOeaTextilRegistroService.getDatosrubroTextil.mockReturnValue(throwError('Error de servicio'));
  ...
  it('debería actualizar autorizacion cuando el valor es numérico', () => {
    component.onRegistroEsquemaCertificacionIVAIEPSChange(1);
    expect(component.autorizacion).toBe(true);

    component.onRegistroEsquemaCertificacionIVAIEPSChange(0);
    expect(component.autorizacion).toBe(false);
  });
});

describe('Autorización 100-A', () => {
  beforeEach(() => {
    component.inicializarFormulario();
    fixture.detectChanges();
  });

  it('debería actualizar el store cuando cambia la autorización', () => {
    const valor = '1';
    
    component.onAutorizacion100AChange(valor);
    
    expect(mockTramite32609Store.actualizarEstado).toHaveBeenCalledWith({
      autorizacion100A: valor
    });
  });

  it('debería manejar valores numéricos para autorización', () => {
    const valor = 1;
...
describe('Funcionalidad de activos fijos', () => {
  beforeEach(() => {
    component.inicializarFormulario();
    fixture.detectChanges();
  });

  it('debería ejecutar opcionActivosFijosFuncion sin errores', () => {
    expect(() => component.opcionActivosFijosFuncion()).not.toThrow();
  });
});
...
describe('Notificaciones', () => {
  ...
  it('debería tener configurada la propiedad de notificación', () => {
    expect(component.esHabilitarElDialogo).toBeDefined();
    expect(component.REQUISITO_OBLIGATORIO).toBeDefined();
  });

  it('debería permitir configurar nuevas notificaciones', () => {
    const notificacion = {
    ...
    component.nuevaNotificacion = notificacion;
    
    expect(component.nuevaNotificacion).toEqual(notificacion);
  });
});

describe('Controladores de estado booleanos', () => {
  beforeEach(() => {
    component.inicializarFormulario();
    fixture.detectChanges();
  });

  it('debería manejar cambios en comercioExteriorActivo', () => {
    component.comercioExteriorActivo = true;
    expect(component.comercioExteriorActivo).toBe(true);

    component.comercioExteriorActivo = false;
    expect(component.comercioExteriorActivo).toBe(false);
  });

  it('debería manejar cambios en parteGrupoComercioExterior', () => {
    component.parteGrupoComercioExterior = true;
    expect(component.parteGrupoComercioExterior).toBe(true);
  });

  it('debería manejar cambios en esFusionOEscisionConComercioExterior', () => {
    component.esFusionOEscisionConComercioExterior = true;
    expect(component.esFusionOEscisionConComercioExterior).toBe(true);
  });

  it('debería manejar cambios en esEmpresaExtranjeraIMMEX', () => {
    component.esEmpresaExtranjeraIMMEX = true;
    expect(component.esEmpresaExtranjeraIMMEX).toBe(true);
  });

  it('debería manejar cambios en registroEsquemaCertificacion', () => {
    component.registroEsquemaCertificacion = true;
    expect(component.registroEsquemaCertificacion).toBe(true);
  });

  it('debería manejar cambios en tipoInformacionEmpresa', () => {
    component.tipoInformacionEmpresa = true;
    expect(component.tipoInformacionEmpresa).toBe(true);
  });
});

describe('Gestión de transportistas', () => {
  beforeEach(() => {
    fixture.detectChanges();
  });

  it('debería inicializar lista de transportistas vacía', () => {
    expect(component.transportistasLista).toEqual([]);
  });

  it('debería tener configuración de columnas para transportistas', () => {
    expect(component.transportistasConfiguracionColumnas).toBeDefined();
    expect(Array.isArray(component.transportistasConfiguracionColumnas)).toBe(true);
  });

  it('debería permitir agregar transportistas a la lista', () => {
    const transportista = {
    ...
    component.transportistasLista.push(transportista);
...
describe('Manejo de errores en fechas', () => {
  it('debería manejar fechas con formato incorrecto', () => {
    expect(ImportadorExportadorComponent.esFechaFutura('2024-13-01')).toBe(false);
    expect(ImportadorExportadorComponent.esFechaFutura('31/02/2024')).toBe(false);
    expect(ImportadorExportadorComponent.esFechaFutura('abc/def/ghij')).toBe(false);
  });

  it('debería manejar fechas null o undefined', () => {
    expect(ImportadorExportadorComponent.esFechaFutura(null as any)).toBe(false);
    expect(ImportadorExportadorComponent.esFechaFutura(undefined as any)).toBe(false);
  });
});

describe('Casos edge en validaciones', () => {
  beforeEach(() => {
    component.inicializarFormulario();
    fixture.detectChanges();
  });

  it('debería manejar RFC con espacios en blanco', () => {
    component.tablaDatos = mockEmpresasDelGrupo;
    
    expect(component.existeRFCEnTabla('   RFC123456789   ')).toBe(true);
    expect(component.existeRFCEnTabla('  ')).toBe(false);
  });

  it('debería manejar tabla vacía en verificación de RFC', () => {
    component.tablaDatos = [];
    
    expect(component.existeRFCEnTabla('RFC123456789')).toBe(false);
  });

  it('debería manejar empresas sin RFC en la tabla', () => {
    component.tablaDatos = [
      { ...mockEmpresasDelGrupo[0], rfcEnclaveOperativo: '' },
      { ...mockEmpresasDelGrupo[1], rfcEnclaveOperativo: null as any }
    ];
...
describe('Limpieza y destrucción', () => {
  it('debería limpiar suscripciones correctamente', () => {
  ...
  it('debería manejar destroy$ undefined', () => {
    component.destroy$ = undefined as any;
    
    expect(() => component.ngOnDestroy()).not.toThrow();
  });
});

describe('Estados del formulario avanzados', () => {
  beforeEach(() => {
    component.inicializarFormulario();
    fixture.detectChanges();
  });

  it('debería validar campos requeridos en formulario principal', () => {
    ...
    requiredFields.forEach(field => {
      const control = component.importadorExportadorForm.get(field);
      control?.setValue('');
  ...
  it('debería validar campos requeridos en formulario de enlace operativo', () => {
  ...
  it('debería mantener campos deshabilitados después de inicialización', () => {
    ...
    disabledFields.forEach(field => {
      expect(component.agregarEnlaceOperativoForm.get(field)?.disabled).toBe(true);
    });
  });
});

describe('Integración completa del estado', () => {
  beforeEach(() => {
    fixture.detectChanges();
  });

  it('debería sincronizar todos los cambios con el store', () => {
    component.inicializarFormulario();
    // Clear any calls made during initialization
    jest.clearAllMocks();
    
    const newValues = {
      comercioExteriorRealizado: '0',
      fechaDePago: '20/01/2024',
      monto: '75000'
    };

    Object.keys(newValues).forEach(key => {
      component.importadorExportadorForm.get(key)?.setValue(newValues[key as keyof typeof newValues]);
      component.setValoresStore(component.importadorExportadorForm, key);
    });

    expect(mockTramite32609Store.actualizarEstado).toHaveBeenCalledTimes(3);
  });
});
...
describe('Búsqueda de RFC', () => {
  ...
  it('debería realizar búsqueda exitosa y actualizar datos', () => {
    ...
    expect(mockOeaTextilRegistroService.conseguirDatosPorRFC).toHaveBeenCalledWith(rfc);
  });

  it('debería manejar error en búsqueda de RFC', () => {
    const consoleSpy = jest.spyOn(console, 'error').mockImplementation();
    mockOeaTextilRegistroService.conseguirDatosPorRFC.mockReturnValue(throwError('Error de red'));
  ...
  it('debería limpiar campos cuando no se encuentra el RFC', () => {
    mockOeaTextilRegistroService.conseguirDatosPorRFC.mockReturnValue(of({}));
...
describe('Validadores de entrada', () => {
  ...
  describe('Renderizado del template', () => {
    ...
    it('debería mostrar título correcto en modal según modo de edición', () => {
      ...
      // Verificar que el template contiene la lógica de título dinámico
