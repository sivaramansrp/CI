...
import { EMPRESA_DEL_GRUPO, EMPRESA_DEL_GRUPO_CON_FECHA, FECHA_DE_INICIO,FECHA_DE_MADALIDAD_IVVAAEIEPS, FECHA_DE_PAGO, INFORMACION_EMPRESA_OPTIONS, OPCIONES_DE_BOTON_DE_RADIO, PANELS, PANELS1, PANELS2, REGISTRO_ESQUEMA_CERTIFICACION_OPTIONS, TERCEROS_CONTRATADOS, TRANSPORTISTAS_CONFIGURACION } from '../../enums/oea-tercerizacion-logistica-registro.enum';
import { EmpresaDelGrupo, RFCEnlaceOperativo, TercerosContratados, TransportistasTable } from '../../modelos/oea-tercerizacion-logistica-registro.model';
...
import { FECHA_DELA_ULTIMA_OPERACION } from'../../enums/oea-tercerizacion-logistica-registro.enum';
import { InputRadioComponent } from '@libs/shared/data-access-user/src';
import { OeaTercerizacionLogisticaRegistroService } from '../../services/oea-tercerizacion-logistica-registro.service';
import { RFCTCEnlaceOperativo } from '../../modelos/oea-textil-registro.model';
...
export class ImportadorExportadorComponent implements OnInit, OnDestroy {
  ...
  /**
   * Formulario reactivo para gestionar la información de terceros contratados.
   * Incluye campos como RFC, denominación social, domicilio y fechas de operación.
   */
  terceroContratadoForm!: FormGroup;
   ...
   */
  tramite32617State!: Tramites32617State;
  ...
  /**
   * Configuración del input de fecha de modalidad IVA y IEPS.
   * Define el formato y validaciones para fechas de modalidad.
   */
  modalidadIVAAeIEPSFecha: InputFecha = FECHA_DE_MADALIDAD_IVVAAEIEPS;
   ...
   * Define la estructura y formato de las columnas a mostrar.
   */
  tableHeaderDatosTC: ConfiguracionColumna<TercerosContratados>[] = TERCEROS_CONTRATADOS;
  ...
  /**
   * Lista de terceros contratados que se muestra en la tabla.
   * Array que contiene todos los terceros contratados agregados con sus datos.
   */
  tablaDatosTC: TercerosContratados[] = [];
   ...
   * Configurado para usar checkboxes en la selección de filas.
   */
  tablaSeleccionCheckboxTC: TablaSeleccion = TablaSeleccion.CHECKBOX;
  ...
  /**
   * Tercero contratado actualmente seleccionado en la tabla.
   * Se utiliza para operaciones de edición y eliminación.
   */
  selectedTC: TercerosContratados | null = null;
   ...
   * Template para mostrar formularios de empresa.
   */
  @ViewChild('templateTC') templateTC!: TemplateRef<void>;
   ...
   * Template para confirmar la eliminación de empresas.
   */
  @ViewChild('template2ConfirmacionEliminacion') template2ConfirmacionEliminacion!: TemplateRef<void>;
  ...
  constructor( 
    ...
    private solicitudService: OeaTercerizacionLogisticaRegistroService,
  ...
  inicializarFormulario(): void {
    ...
    this.importadorExportadorForm = this.fb.group({
      comercioExteriorRealizado : [this.tramite32617State?.comercioExteriorRealizado, Validators.required],
      fechaDePago: [this.tramite32617State?.fechaDePago, Validators.required],
      fechaInicioComercio: [this.tramite32617State?.fechaInicioComercio],
      esParteGrupoComercioExterior: [this.tramite32617State?.esParteGrupoComercioExterior],
      fusionEscisionConOperacionExterior: [this.tramite32617State?.fusionEscisionConOperacionExterior],
      empresaExtranjeraIMMEX: [this.tramite32617State?.empresaExtranjeraIMMEX],
      monto:[ this.tramite32617State?.monto, Validators.required],
      operacionesBancarias:[this.tramite32617State?.operacionesBancarias, Validators.required],
      llavePago: [this.tramite32617State?.llavePago, Validators.required],
      registroEsquemaCertificacion: [this.tramite32617State?.registroEsquemaCertificacion, Validators.required],
      tipoInformacionEmpresa: [this.tramite32617State?.tipoInformacionEmpresa, Validators.required],
      programaIMMEX: [this.tramite32617State?.programaIMMEX, Validators.required],
      modalidadIVAAeIEPS: [this.tramite32617State?.modalidadIVAAeIEPS, Validators.required],
      documentaciónDeComercioExterior: [this.tramite32617State?.documentaciónDeComercioExterior, Validators.required],
      utilizandoSuPropioTransporte: [this.tramite32617State?.utilizandoSuPropioTransporte, Validators.required],
      serviciosTercerosConCertificacion: [this.tramite32617State?.serviciosTercerosConCertificacion, Validators.required],
      modalidadIVAAeIEPSFecha: [{ value: this.tramite32617State?.modalidadIVAAeIEPSFecha, disabled: true }],
      modalidadIVAAeIEPSValor: [{value: this.tramite32617State?.modalidadIVAAeIEPSValor, disabled: true}],
      numeroDeAutorizacion: [{ value: this.tramite32617State?.numeroDeAutorizacion, disabled: true }]
    })
    this.agregarEnlaceOperativoForm = this.fb.group({
      rfcEnclaveOperativo:[this.tramite32617State?.rfcEnclaveOperativo, Validators.required],
      enlaceOperativorfc: [
        {value:this.tramite32617State?.enlaceOperativorfc, disabled: true}
      ],
      denominacionRazonsocial: [
        {value:this.tramite32617State?.denominacionRazonsocial, disabled: true}
      ],
      domicilio: [{value:this.tramite32617State?.domicilio, disabled: true}],
      inputfechaDeLaUltimaOperacion: [{value:this.tramite32617State?.inputfechaDeLaUltimaOperacion, disabled: true}],
    });

    this.terceroContratadoForm = this.fb.group({
      rfcTerceroContratado: [this.tramite32617State?.rfcTerceroContratado, Validators.required],
      rfcTC: [{value: this.tramite32617State?.rfcTC, disabled: true}],
      nombreDenominacionTC: [{value: this.tramite32617State?.nombreDenominacionTC, disabled: true}],
      ctpatCertificadoTC: [{value: this.tramite32617State?.ctpatCertificadoTC}],
      registroEsquemaCertificacionTC: [{value: this.tramite32617State?.registroEsquemaCertificacionTC}],
      registroSocioComercialCertificadoTC: [{value: this.tramite32617State?.registroSocioComercialCertificadoTC}],
    });

    if (this.tramite32617State?.tablaDatos) {
      this.tablaDatos = [...this.tramite32617State.tablaDatos];
    }
    if(this.tramite32617State?.tablaDatosTC) {
      this.tablaDatosTC = [...this.tramite32617State.tablaDatosTC];
    }
   ...
   * Cierra el modal, limpia el formulario y resetea el modo de edición.
   */
  cancelarModalTC(): void {
    this.modalRefabir?.hide();
    this.terceroContratadoForm.reset();
    this.resetEditMode();
  }
   ...
   * param template - Template del modal a mostrar
   */
  terceroContratadoModal(template: TemplateRef<void>): void {
    this.modalRefabir = this.modalService.show(template, { class: 'modal-lg',});
  }
   ...
   */
  buscarRFCTC(): void {
    const RFC = this.terceroContratadoForm.get('rfcTerceroContratado')?.value;
    if (RFC) {
      // Verificar si el RFC ya existe en la tabla
      if (this.existeRFCTCEnTabla(RFC)) {
        this.mostrarModalRFCDuplicado();
        return;
      }

      this.buscarDatosPorRFCTC(RFC);
    }
  }
   ...
   * returns true si el RFC existe, false en caso contrario
   */
  existeRFCTCEnTabla(rfc: string): boolean {
    return this.tablaDatosTC.some(datos => 
      datos.rfcTerceroContratado?.toLowerCase() === rfc.toLowerCase()
    );
  }
   ...
   * Ejecuta la consulta al servicio para obtener datos adicionales.
   */
  buscarDatosPorRFCTC(rfc: string): void {
    this.solicitudService.conseguirDatosPorRFCTC(rfc)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (datos) => {
          const EMPRESA_DATA = datos[rfc];
          if (EMPRESA_DATA) {
            this.patchearDatosTC(EMPRESA_DATA);
          } else {
            this.limpiarCamposEmpresa();
          }
        },
   ...
   * param empresaData - Datos de la empresa obtenidos del servicio
   */
  patchearDatosTC(empresaData: RFCTCEnlaceOperativo): void {
    this.terceroContratadoForm.patchValue({
      rfcTC: empresaData.rfcTC,
      nombreDenominacionTC: empresaData.nombreDenominacionTC
    });
  }
   ...
   * Maneja tanto el modo de adición como el de edición de empresas.
   */
  aceptarTerceroContratado(): void {
    const RFC_VALUE = this.terceroContratadoForm.get('rfcTerceroContratado')?.value;

    // Verificar que el campo RFC requerido esté lleno
    if (!RFC_VALUE?.trim()) {
      this.terceroContratadoForm.get('rfcTerceroContratado')?.markAsTouched();
      return;
    }
    
    // En modo edición, verificar si el RFC existe en otros registros (excluyendo el actual)
    if (this.isEditMode && this.existeRFCTCEnTablaExcluyendo(RFC_VALUE, this.selectedTC?.rfcTerceroContratado)) {
      this.mostrarModalDatosObligatorios();
      return;
    }
    // En modo agregar, verificar si el RFC ya existe
    if (!this.isEditMode && this.existeRFCTCEnTabla(RFC_VALUE)) {
      this.mostrarModalDatosObligatorios();
      return;
    }
    
    // Proceder con agregar los datos
    const FORMDATOS = this.terceroContratadoForm.getRawValue();

    const EMPRESA_DATA: TercerosContratados = {
      rfcTerceroContratado: FORMDATOS.rfcTerceroContratado,
      denominacionRazonsocial: FORMDATOS.nombreDenominacionTC,
      tipoRegistro: 'C-TPAT-SCC',
      ctpatCertificadoTC: FORMDATOS.ctpatCertificadoTC,
      registroEsquemaCertificacionTC: FORMDATOS.registroEsquemaCertificacionTC,
      registroSocioComercialCertificadoTC: FORMDATOS.registroSocioComercialCertificadoTC
    };

    if (this.isEditMode && this.selectedTC) {
      const INDEX = this.tablaDatosTC.findIndex(emp => 
        emp.rfcTerceroContratado === this.selectedTC?.rfcTerceroContratado
      );
      if (INDEX !== -1) {
        this.tablaDatosTC[INDEX] = EMPRESA_DATA;
        this.tablaDatosTC = [...this.tablaDatosTC]; 
      }
    } else {
      this.tablaDatosTC = [...this.tablaDatosTC, EMPRESA_DATA];
    }
    
    this.actualizarTablaDatosTCEnStore();

    // Mostrar la columna de fecha después de agregar el primer elemento
    if (!this.mostrarColumnaFecha) {
      this.mostrarColumnaFecha = true;
      this.tableHeaderDatosTC = TERCEROS_CONTRATADOS;
    }
    
    this.limpiarFormularioTC();
    this.isEditMode = false;
    this.limpiarSeleccionTC();
    this.modalRefabir?.hide();
    this.mostrarModalExito();
  }
   ...
   * returns true si existe el RFC (excluyendo el especificado), false en caso contrario
   */
  existeRFCTCEnTablaExcluyendo(rfc: string, rfcExcluir?: string): boolean {
    return this.tablaDatosTC.some(datos => 
      datos.rfcTerceroContratado?.toLowerCase() === rfc.toLowerCase() &&
      datos.rfcTerceroContratado?.toLowerCase() !== rfcExcluir?.toLowerCase()
    );
  }
   ...
   * Resetea todos los campos a valores vacíos y desactiva el modo edición.
   */
  limpiarFormularioTC(): void {
    this.terceroContratadoForm.reset();
    this.terceroContratadoForm.patchValue({
      rfcTC: '',
      nombreDenominacionTC: '',
      domicilioTC: '',
      fechaInicioTC: '',
      fechaFinTC: ''
    });
   ...
   * param empresa - Empresa seleccionada de la tabla
   */
  onFilaSeleccionadaTC(tercerosContratados: TercerosContratados): void {
    this.selectedTC = tercerosContratados;
  }
  ...
  /**
   * Inicia el proceso de modificación de un tercero contratado existente.
   * Valida que haya un tercero contratado seleccionado y configura el modo de edición.
   * Rellena el formulario con los datos del tercero contratado seleccionado.
   */
  modificarTerceroContratado(): void {
    if (this.tablaDatosTC.length === 0 || !this.selectedTC) {
      this.mensajeSeleccion = 'Debe seleccionar un elemento';
      this.mostrarModalSeleccionRequerida();
      return;
    }
    ...
    this.terceroContratadoForm.patchValue({
      rfcTerceroContratado: '',
      rfcTC: this.selectedTC.rfcTerceroContratado,
      nombreDenominacionTC: this.selectedTC.denominacionRazonsocial,
      ctpatCertificadoTC: this.selectedTC.ctpatCertificadoTC,
      registroEsquemaCertificacionTC: this.selectedTC.registroEsquemaCertificacionTC,
      registroSocioComercialCertificadoTC: this.selectedTC.registroSocioComercialCertificadoTC
    });
    
    // Abrir el modal
    this.abrirModal(this.templateTC);
  }
  ...
  eliminarEmpresa(): void {
    ...
    this.mostrarModalConfirmacionEliminacion('empresa');
  }
   ...
   * Muestra el modal de confirmación para la eliminación.
   */
  eliminarTerceroContratado(): void {
    if (this.tablaDatosTC.length === 0 || !this.selectedTC) {
      this.mensajeSeleccion = 'Debe seleccionar un elemento';
      this.mostrarModalSeleccionRequerida();
      return;
    ...
    // Mostrar modal de confirmación antes de eliminar
    this.mostrarModalConfirmacionEliminacion('TC');
  }
   ...
   * Actualiza la lista y el store, resetea la selección y muestra confirmación.
   */
  confirmarEliminacionTC(): void {
    if (!this.selectedTC) {
      return;
    }
    
    // Eliminar la empresa seleccionada de la tabla
    this.tablaDatosTC = this.tablaDatosTC.filter(datos => 
      datos.rfcTerceroContratado !== this.selectedTC?.rfcTerceroContratado
    );
   ...
   */
  mostrarModalConfirmacionEliminacion(datos:string): void {
    const MODAL_CONFIG = {
      animated: true,
      keyboard: false,
      backdrop: true,
    ...
    this.modalRef = this.modalService.show(datos === 'empresa' ? this.templateConfirmacionEliminacion : this.template2ConfirmacionEliminacion, MODAL_CONFIG);
  }
   ...
   * param index - Índice del panel a mostrar/ocultar
   */
  mostrar_colapsable2(index: number): void {
    const IS_CURRENTLY_OPEN = this.panels2[index].isCollapsed;
    this.panels2.forEach((panel, i) => {
      panel.isCollapsed = i === index ? !IS_CURRENTLY_OPEN : true;
    });
  }
 ...
 validarCamposPago(): void {
  ...
  obtenerEstadoSolicitud(): void {
    this.tramite32617Query.selectTramite32617$?.pipe(takeUntil(this.destroy$))
      .subscribe((data: Tramites32617State) => {
        this.tramite32617State = data;
        if (data.tablaDatos) {
  ...
  actualizarTablaDatosEnStore(): void {
    this.tramite32617Store.establecerDatos({ tablaDatos: this.tablaDatos });
  }
   ...
   * Sincroniza los cambios locales con el estado global de la aplicación.
   */
  actualizarTablaDatosTCEnStore(): void {
    this.tramite32617Store.establecerDatos({ tablaDatosTC: this.tablaDatosTC });
  }
