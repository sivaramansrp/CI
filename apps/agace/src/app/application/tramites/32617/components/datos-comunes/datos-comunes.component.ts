...
import { NOTA, OPCIONES_DE_BOTON_DE_RADIO } from '../../enums/oea-tercerizacion-logistica-registro.enum';
...
export class DatosComunesComponent implements OnInit, OnDestroy {
  ...
  /**
   * Indica si hay registros disponibles.
   */
  sonRegistros:boolean = false;
   ...
   /**
    ...
    return;
      ...
      this.forma = this.fb.group({
          ...
          registros: [{value: this.seccionState?.registros, disabled: true}]
...
private actualizarFormularioConDatosDelEstado(): void {
  if (this.forma && this.seccionState && this.esFormularioInicializado) {
    ...
    const STATEVALOR = {
      ...
      registros: this.seccionState.registros,
    ...
    manejarCamposMutuamenteExcluyentes(
      ...
      if (VALOR) {
        ...
        this.tramite32617Store.establecerDatos({ [campoAResetear]: null });
      }
    }  
...
/**
   ...
   *
   * @param {[string,string]} value - El valor seleccionado para reconocimiento mutuo CTPAT.
   */
  onReconocimientoMutuoCTPATChanged(value: [string,string]): void {
    this.reconocimientoMutuoCTPATChange.emit(value);
  }

  anexarArchivo(): void {
    this.sonRegistros = true;
  }
