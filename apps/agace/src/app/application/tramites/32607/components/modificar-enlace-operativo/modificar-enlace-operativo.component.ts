...
/**
 * Componente para modificar un enlace operativo.
 * Utiliza un formulario reactivo para capturar y emitir la información del enlace operativo.
 */
@Component({
  selector: 'app-modificar-enlace-operativo',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, TituloComponent],
  templateUrl: './modificar-enlace-operativo.component.html',
  styleUrl: './modificar-enlace-operativo.component.scss',
})
/**
 * Componente para modificar un enlace operativo.
 * Utiliza un formulario reactivo para capturar y emitir la información del enlace operativo.
 */
export class ModificarEnlaceOperativoComponent implements OnInit, OnDestroy {
  /** Formulario reactivo para capturar los datos del enlace operativo */
  modificarEnlaceOperativoForm!: FormGroup;
  ...
  guardarDatosFormulario(): void {
    this.inicializarFormulario();
    if (this.esFormularioSoloLectura) {
      this.modificarEnlaceOperativoForm.disable();
    } else if (!this.esFormularioSoloLectura) {
      this.modificarEnlaceOperativoForm.enable();
    } else {
  ...
  inicializarFormulario(): void {
    this.modificarEnlaceOperativoForm = this.fb.group({
    ...
    this.solicitud32607Query.selectSolicitud$
      .pipe(
        ...
        map((respuesta: Solicitud32607State) => {
          ...
          this.modificarEnlaceOperativoForm.patchValue({
  ...
  buscarTerceroNacionalIDC(): void {
    if (this.modificarEnlaceOperativoForm.get('rfcTercero')?.value) {
  ...
  aceptarEnlaceSuplente(): void {
    const OBJETO_JSON: EnlaceOperativo = {
      rfc: this.modificarEnlaceOperativoForm.get('agregarEnlaceRfc')?.value,
      nombre: this.modificarEnlaceOperativoForm.get('agregarEnlaceNombre')?.value,
      apellidoPaterno: this.modificarEnlaceOperativoForm.get(
        'agregarEnlaceApellidoPaterno'
      )?.value,
      apellidoMaterno: this.modificarEnlaceOperativoForm.get(
        'agregarEnlaceApellidoMaterno'
      )?.value,
      claveCiudad: '',
      ciudad: this.modificarEnlaceOperativoForm.get('agregarEnlaceCiudadEstado')
        ?.value,
      cargo: this.modificarEnlaceOperativoForm.get('agregarEnlaceCargo')?.value,
      telefono: this.modificarEnlaceOperativoForm.get('agregarEnlaceTelefono')
        ?.value,
      correo: this.modificarEnlaceOperativoForm.get(
        'agregarEnlaceCorreoElectronico'
      )?.value,
      suplente:
        this.modificarEnlaceOperativoForm.get('agregarEnlaceSuplente')?.value ===
        true
          ? 'Maria López'
          : '',
      calle: '',
  ...
  noEsValido(id: string): boolean | undefined {
    const CONTROL = this.modificarEnlaceOperativoForm.get(id);
