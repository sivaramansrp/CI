 ...
 */
   ...
   * @param solicitud33304Store Almacén para manejar el estado de la aplicación.
   * @param unicoQuery Consultas para obtener el estado actual de la aplicación.
   */

   constructor(private fb: FormBuilder,private service: AvisoUnicoService,private solicitud33304Store: Solicitud33304Store,
      private unicoQuery: Solicitud33304Query,private consultaioQuery: ConsultaioQuery) {}
    ...
    inicializamiFormulario(): void {
      this.service.getAvisoModify()
        .pipe(
          takeUntil(this.destroy$),
          map((resp) => {
            this.solicitud33304Store.setModalidadCertificacion(resp.descripcion);
          })
        )
        .subscribe();
    }
...
/**
   ...
   */
  setValoresStore(form: FormGroup, campo: string, metodoNombre: keyof Solicitud33304Store): void {
    const VALOR = form.get(campo)?.value;
    (this.solicitud33304Store[metodoNombre] as (value: string) => void)(VALOR);
  }
  /**
