...
describe('DatosMercanciaComponent', () => {
  ...
  beforeEach(async () => {
    ...
    mockDatosMercanciaService.obtenerSelectorList.mockImplementation(() => of([{ id: 1, descripcion: 'Test' }]));
  ...
  it('should fetch paisorigen list and set catalogoPaisOrigen', () => {
    ...
    expect(mockDatosMercanciaService.obtenerSelectorList).toHaveBeenCalledWith('paisorigen.json');
  ...
  it('should fetch nombrecomun list and set catalogoNombreComun', () => {
    ...
    expect(mockDatosMercanciaService.obtenerSelectorList).toHaveBeenCalledWith('nombrecomun.json');
  ...
  it('should fetch nombrecientifico list and set catalogoNombreCientifico', () => {
    ...
    expect(mockDatosMercanciaService.obtenerSelectorList).toHaveBeenCalledWith('nombrecientifico.json');
...
it('should add item to cuerpoTabla when nombre is AGREGAR', () => {
  ...
  jest.spyOn(component as any, 'limpiarDatosFormulario').mockImplementation(() => {});
  jest.spyOn((component as any).datosMercanciaService, 'actualizarFormularioMovilizacion');
...
it('should reset formMercancia when limpiarDatosFormulario is called', () => {
  ...
  expect(component.formMercancia.pristine).toBe(true);
});
...
it('should remove the selected item from cuerpoTabla and update service when only one item exists', () => {
  ...
  const actualizarSpy = jest.spyOn((component as any).datosMercanciaService, 'actualizarFormularioMovilizacion');
  const desactivarSpy = jest.spyOn((component as any).datosMercanciaService, 'botonDesactivarCampos');
...
it('should set estadoChecker to true and patch the form with selected item data', () => {
  ...
  // Assertions
  expect(component.estadoChecker).toBe(true);
