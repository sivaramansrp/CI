...
export const MERCANCIAS_COLUMNA = [
  {
    encabezado: 'Descripción de la mercancía',
    clave: (item : DescripcionMercancia ) => item.descripcionMercancia,
    orden: 1
  }
];
...
export const CONFIGURACION_COLUMNA = [
  {
    ...
    clave: (item: ClavesDePermisos) => item.certificado,
    orden: 1,
  },
  {
    encabezado: 'Fecha expedición',
    clave: (item: ClavesDePermisos) => item.fechaExpedicion,
    orden: 2,
  },
  {
    encabezado: 'Tipo de movimiento',
    clave: (item: ClavesDePermisos) => item.tipoMovimiento,
    orden: 3,
  },
];
