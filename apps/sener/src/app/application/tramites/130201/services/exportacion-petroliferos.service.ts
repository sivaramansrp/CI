 ...
 * 
 * @export
 * @class ExportacionPetroliferosService
 */
@Injectable({
  providedIn: 'root',
})
export class ExportacionPetroliferosService {

  /**
   * Crea una instancia del servicio `ExportacionPetroliferosService`.
  ...
  getListaDePaisesDisponibles(): Observable<Catalogo[]> {
    return this.http.get<Catalogo[]>('/assets/json/130201/pais-procenia.json');
  }
  ...
  obtenerListaDeCiudades(): Observable<Catalogo[]> {
    return this.http.get<Catalogo[]>('/assets/json/130201/todas-las-ciudades.json');
  }
  ...
  getPaisesPorBloque(_bloqueId: number): Observable<Catalogo[]> {
    return this.http.get<Catalogo[]>('/assets/json/130201/paises-por-bloque.json');
  }
  ...
  getEstado(): Observable<Catalogo[]> {
    return this.http.get<Catalogo[]>('/assets/json/130201/estado.json');
  }
  ...
  getRepresentacionFederal(): Observable<Catalogo[]> {
    return this.http.get<Catalogo[]>('/assets/json/130201/representacion-federal.json');
  }
  ...
  getSolicitudeOptions(): Observable<ProductoResponse> {
    return this.http.get<ProductoResponse>('assets/json/130201/solicitude-options.json');
  }
  ...
  getProductoOptions(): Observable<ProductoResponse> {
    return this.http.get<ProductoResponse>('assets/json/130201/plazo-options.json');
  }
 ...
 * @example
 * // Ejemplo de uso:
 * this.ExportacionPetroliferosService.getTablaDatos().subscribe((datos) => {
 *   console.log(datos);
 * });
 */
  getTablaDatos(): Observable<PartidasDeLaMercanciaModelo[]> {
    return this.http.get<PartidasDeLaMercanciaModelo[]>(
          'assets/json/130201/partidas-de-la.json'
        );
  }
}
