...
// import { ModalMateriaPrimaService } from './modalMateriaPrima.service';
import { Observable } from 'rxjs';

@Component({
  selector: 'app-modal-residuos',
  templateUrl: './modal-residuos.component.html',
  styleUrls: ['./modal-residuos.component.css']
})
export class ModalResiduosComponent implements OnInit {
  materiaPrimaForm: FormGroup;
  showMessage: boolean = false;
  mostrarMsgCantSe06: boolean = false;

  // Dropdown lists
  comboUnidadMedida: Array<{ clave: string, descripcion: string }> = [];
  comboCapituloFraccion: Array<{ clave: string, descripcion: string }> = [];
  comboPartidaFraccion: Array<{ clave: string, descripcion: string }> = [];
  comboSubPartidaFraccion: Array<{ clave: string, descripcion: string }> = [];
  comboFraccionArancelariaParametros: Array<{ clave: string, descripcion: string }> = [];

  constructor(
    private fb: FormBuilder,
    private modalMateriaPrimaService: ModalMateriaPrimaService
  ) {
    this.materiaPrimaForm = this.fb.group({
      ...
      cantidadEnLetra: [{ value: '', disabled: true ,max: 256}],
  ...
  cambiaFraccion(): void {
    const fraccionSeleccionada = this.comboFraccionArancelariaParametros.find(fr => fr.clave === this.materiaPrimaForm.get('fraccion')?.value);
    if (fraccionSeleccionada) {
      this.materiaPrimaForm.patchValue({
        descFraccion: fraccionSeleccionada.descripcion,
        generica1: fraccionSeleccionada.clave
      });
      this.validaVigenciaFraccion(fraccionSeleccionada.clave);
    }
  }

  cambiaUnidadMedida(): void {
    const unidadSeleccionada = this.comboUnidadMedida.find(unidad => unidad.clave === this.materiaPrimaForm.get('unidadMedidaComercial.clave')?.value);
  ...
  validaVigenciaFraccion(clvFracion: string): void {
