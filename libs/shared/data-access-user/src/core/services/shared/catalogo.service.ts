...
import { CATALOGO_ESTADOS, CATALOGO_IMMEX, CATALOGO_NICO, CATALOGO_PAISES, CATALOGO_PAISES_BLOQUE, CATALOGO_TRATADOS_ACUERDOS, COMUN_URL } from '../../servers/api-router';
...
export class CatalogoServices {
   ...
   */
  tratadosAcuerdosCatalogo(tramite: string): Observable<BaseResponse<Catalogo[]>> {
    const ENDPOINT = `${this.host}${CATALOGO_TRATADOS_ACUERDOS(tramite)}`;
    return this.http.get<BaseResponse<Catalogo[]>>(ENDPOINT);
  }
