 ...
 */
  ...
  tratadoCatalogoPais(tramite: string): Observable<BaseResponse<Catalogo[]>> {
    ...
    if (tramite === '110207') {
      const ENDPOINT = `${this.host}${CATALOGO_TRATADO_ACUERDO_PAIS_110207(tramite)}`;
      return this.http.get<BaseResponse<Catalogo[]>>(ENDPOINT);
    }
